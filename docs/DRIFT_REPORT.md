...
## Checkpoint applicatif — 4 septembre 2026, 16:43 UTC

- Branche : `codex/tradikom-one-os`; head distant publié `28efa750935b2766de3410b8d9c0d5e3c4e2dbe8`; contrôle d'intégrité conservé localement dans `3df4f7c5f642bfca473820a4efe1f42838c3a5c5` et scénario UI négatif dans `15b2af7949e937b2f59a5cdd6db9728489fb1478`. Après le handoff documentaire, la branche sera en avance de douze commits, sans push.
- Travail effectué : recalcul SHA-256 à chaque lecture du texte extrait, suppression fail-closed du contenu altéré ou partiel, contrat `verified`/`failed`, messages français et vue `data-only` filtrée et bornée pour un futur consommateur.
- Impact north star : la conversation n'affiche plus une extraction persistée sans en vérifier l'intégrité. Une altération devient immédiatement visible sans divulguer le texte; une future consommation automatique dispose d'une frontière de données explicite sans pouvoir fournir règle, outil ou mutation de politique.
- Risques contenus : la vue `data-only` masque URLs, adresses privées, jetons Bearer, affectations de secrets et blocs PEM; elle n'est appelée par aucun code de production. Aucun LLM, outil, OCR, fournisseur, Graph, stockage Supabase, secret, endpoint public, message, activation, fusion, déploiement ou dépense. `tmp/` reste non suivi et hors index.
...
| --- | --- | --- | --- |
| 3-7, 31-33, 46, 48, 70-71 | Conversation-first, tranche verticale utile, ordre strict et reprise exacte | Lecture canonique durcie sans nouveau module métier; quatre documents de reprise actualisés; aucun CRM/Kanban/dashboard/OS-6 | Publication et CI complète soumises à l'ordre explicite « publie et lance la CI » |
| 10-18, 64-65 | Hub canonique, frontière provider, policy, idempotence, activité durable et états honnêtes | Le contrat de sortie distingue intégrité vérifiée et échec fermé; l'entrée ne peut toujours pas usurper l'extraction; aucun fournisseur ou outil ajouté | Provider Graph, bootstrap, stockage, scanner et extracteur réels non autorisés ou non configurés |
| 22-24, 35-38 | Tenant/RLS, fichier externe non fiable, hachage, minimisation et défense contre l'injection | SHA-256 recalculé à chaque lecture; texte absent si altéré/partiel; vue `data-only` bornée à 8 000 caractères, URLs/adresses/secrets/PEM masqués et capacités interdites; états français visibles | PostgreSQL/RLS à exécuter en CI; stockage Supabase/ACL, antivirus et OCR/transcription réels non livrés; vue volontairement non raccordée |
| 32, 69 | DoD et matrice unit/intégration/RLS/provider/workflow/Playwright/sécurité | 4 fichiers/40 tests ciblés verts; régression exhaustive 140 fichiers/685 tests verts; ESLint, TypeScript, build, continuity-check direct et diff check verts; scénario Playwright succès+altération adapté | 8 fichiers/21 tests PostgreSQL ignorés sans `DATABASE_URL`; Playwright non exécuté localement; CI requise |

Le PDF canonique est conforme : 71 pages, SHA-256 `bb838fb02c23247b1bcda8981539eebe73264a5334bfaf565aafa5bc26c50fe5`.

## Classification actuelle

- Livré localement : worker média, migrations, rendu Conversation, composition générique, scan de sécurité, extraction `external_untrusted_data`, contrôle d'intégrité à la lecture et vue `data-only` non raccordée, uniquement avec doubles mock.
- Réel connecté : aucun nouveau fournisseur; Graph et stockage réel non composés.
- Sandbox : aucune appelée.
- Mock : fetch, scanner propre/dangereux/temporaire, extracteur et stockage média uniquement via doubles injectés; affichages explicites « Stockage mock », intégrité vérifiée ou extraction masquée.
...
- Livré localement, CI non déclenchée : worker média durable et composition générique, validation binaire/hachage, scanner puis extraction `external_untrusted_data` obligatoires, vérification SHA-256 à la lecture, échec fermé, vue `data-only` non raccordée, stockage/pièce jointe mock idempotents et rendu conversationnel français.
