...
## Checkpoint applicatif — 4 septembre 2026, 21:02 UTC

- Branche : `codex/tradikom-one-os`; head distant publié `28efa750935b2766de3410b8d9c0d5e3c4e2dbe8`; autorisations durables des fils conservées localement dans `8d6e329b363cac96b5cfb793f82abab7e9aee273`. Après le handoff documentaire, la branche sera en avance de vingt-et-un commits, sans push; `tmp/` reste non suivi et hors index.
- Travail effectué : migrations 112/113 et miroirs 0106/0107, droits reliés au fil et au membership par clés composées, RLS et index tenant-leading; configuration atomique/idempotente propriétaire-administrateur; filtrage des listes, lectures et messages humains; contrôle des pièces jointes avant stockage et après IO; badge français.
- Impact north star : l'accès à une conversation et à ses preuves suit maintenant explicitement les utilisateurs autorisés pour `personal`, `team` et `case`, tout en conservant la compatibilité des fils `tenant`.
- Risques contenus : aucun canal ne crée de droit; une révocation pendant l'IO ferme l'accès; l'audit ne reçoit ni contenu, ni identifiant de bénéficiaire, ni clé brute; aucun fournisseur réel ou effet externe n'est composé.
...
| --- | --- | --- | --- |
| 3-7, 31-33, 46-48, 70-71 | Conversation-first, tranche verticale utile, ordre strict et reprise exacte | Droits utilisateur durables ajoutés au Conversation Hub et visibles en français; aucun CRM/Kanban/dashboard/OS-6 | Publication et CI complète soumises à l'ordre explicite « publie et lance la CI » |
| 10-14, 17-18, 64-65 | Un canal n'accorde aucun droit; le fil canonique doit expliquer l'accès; policy avant IO et actions sûres | `personal`/`team`/`case` exigent un grant actif dans listes, lectures et ingestion humaine; `tenant` reste compatible; pièces jointes refusées avant stockage et revalidées après IO | Propager la même frontière aux plans, décisions, exécutions et reprises OS-1/OS-4 |
| 22-24, 35-38 | Tenant/RLS, relations composées, index tenant-leading, minimisation et tests cross-tenant | Migrations 112/113 et miroirs 0106/0107; FK fil/membership du même tenant, cardinalités, unicité, RLS administrateur/utilisateur et registre idempotent immuable | PostgreSQL/RLS réel à exécuter en CI; stockage Supabase/ACL, antivirus et OCR/transcription réels non livrés |
| 32, 69 | DoD et matrice unit/intégration/RLS/provider/workflow/Playwright/sécurité | Finaux après durcissement : 6 fichiers ciblés verts + 1 PostgreSQL ignoré, 27 tests verts + 1 ignoré; ESLint ciblé et TypeScript verts. Avant ce dernier SQL : régression 158 tests et suite exhaustive 722 tests vertes; lint et build verts | 23 tests PostgreSQL de la suite exhaustive ignorés sans `DATABASE_URL`; Playwright et CI du head requis |

Le PDF canonique est conforme : 71 pages, SHA-256 `bb838fb02c23247b1bcda8981539eebe73264a5334bfaf565aafa5bc26c50fe5`.

## Classification actuelle

- Livré localement : worker média, migrations, rendu Conversation, composition générique, scan, extraction `external_untrusted_data`, contrôle d'intégrité, vue `data-only` non raccordée, accès court, frontière HTTP/session, classification confidentialité/visibilité et autorisations durables personal/team/case appliquées aux fils/messages/pièces jointes; doubles mock uniquement pour les fournisseurs.
