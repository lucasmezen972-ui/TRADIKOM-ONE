...
## Checkpoint applicatif — 4 septembre 2026, 18:42 UTC

- Branche : `codex/tradikom-one-os`; head distant publié `28efa750935b2766de3410b8d9c0d5e3c4e2dbe8`; frontière HTTP/session conservée localement dans `e24356f951a46abaf8cd5f95a37362463f778c0f` et preuve verticale dans `422317e8452b3cecea9c0514b8306787e65c6bee`. Après le handoff documentaire, la branche sera en avance de dix-sept commits, sans push; `tmp/` reste non suivi et hors index.
- Travail effectué : route same-origin `POST` préparation + `PUT` consommation, session/tenant avant service, JSON UTF-8 strict borné avec ticket hors URL, réponses françaises sûres, téléchargement borné, headers privés et composition production `not_configured`.
- Impact north star : la capacité courte du fil canonique est désormais utilisable via une frontière HTTP réévaluable par la session, sans transformer une référence de stockage en URL technique ni simuler un fournisseur réel.
- Risques contenus : origine, méthode, session, tenant, identifiant, type et taille du corps échouent fermé; ticket, contenu, nom, checksum et référence restent hors URL, réponse d'erreur et audit. Seuls des doubles `mock` injectés dans les tests réalisent une lecture.
...
| --- | --- | --- | --- |
| 3-7, 31-33, 46, 48, 70-71 | Conversation-first, tranche verticale utile, ordre strict et reprise exacte | Frontière HTTP utilisable ajoutée au Conversation Hub, états français explicites et quatre documents actualisés; aucun CRM/Kanban/dashboard/OS-6 | Publication et CI complète soumises à l'ordre explicite « publie et lance la CI » |
| 10-18, 64-65 | Hub canonique, adaptateur mince, policy, idempotence et états provider honnêtes | La route délègue au service; production `not_configured`; `POST` prépare et `PUT` consomme sans logique provider ni IO externe | Confidentialité/visibilité durables du fil à transmettre à la policy; Graph et stockage réel non autorisés/non configurés |
| 22-24, 35-38 | Droits hérités, fichier externe non fiable, accès court, minimisation et défense contre la substitution | Origine/session/tenant réévalués; JSON/ticket bornés et hors URL; `no-store`, `no-referrer`, `nosniff`; nom de fichier assaini | Confidentialité/visibilité durable, PostgreSQL/RLS en CI, stockage Supabase/ACL, antivirus et OCR/transcription réels non livrés |
| 32, 69 | DoD et matrice unit/intégration/RLS/provider/workflow/Playwright/sécurité | 17 tests HTTP + 1 intégration; ciblés finaux 3 fichiers/26 tests; suite exhaustive 142 fichiers/710 tests; ESLint, TypeScript, build, continuity-check et diff check verts | 8 fichiers/21 tests PostgreSQL ignorés sans `DATABASE_URL`; Playwright attend PostgreSQL partagé; CI requise |

Le PDF canonique est conforme : 71 pages, SHA-256 `bb838fb02c23247b1bcda8981539eebe73264a5334bfaf565aafa5bc26c50fe5`.

## Classification actuelle

- Livré localement : worker média, migrations, rendu Conversation, composition générique, scan, extraction `external_untrusted_data`, contrôle d'intégrité, vue `data-only` non raccordée, accès court chiffré et frontière HTTP/session des pièces jointes, uniquement avec doubles mock.
- Réel connecté : aucun nouveau fournisseur; Graph et stockage réel non composés.
- Sandbox : aucune appelée.
- Mock : fetch, scanner propre/dangereux/temporaire, extracteur, stockage média, codec et lecture HTTP uniquement via doubles injectés; affichages explicites « Stockage mock », intégrité vérifiée/extraction masquée et téléchargement non configuré.
