...
## Checkpoint applicatif — 4 septembre 2026, 19:41 UTC

- Branche : `codex/tradikom-one-os`; head distant publié `28efa750935b2766de3410b8d9c0d5e3c4e2dbe8`; classification des fils conservée localement dans `8900ac5836c504c970d32ee88f8f87bdb515a591`. Après le handoff documentaire, la branche sera en avance de dix-neuf commits, sans push; `tmp/` reste non suivi et hors index.
- Travail effectué : migration additive 111 et miroir 0105 pour confidentialité/visibilité, contraintes et index tenant-leading; propagation dans le contrat et la lecture Conversation; badges français; héritage obligatoire par la policy des pièces jointes et revalidation post-IO.
- Impact north star : le fil canonique porte enfin des droits explicites et compréhensibles qui suivent ses pièces jointes, au lieu de réduire l'accès à la seule appartenance générale à l'organisation.
- Risques contenus : seules les valeurs bornées sont persistées; les anciens fils deviennent `internal`/`tenant`; les jointures restent tenant-scoped; une mutation des droits pendant l'IO échoue fermé. Aucun droit, contenu, ticket ou référence de stockage n'est ajouté à l'audit.
...
| --- | --- | --- | --- |
| 3-7, 31-33, 46, 48, 70-71 | Conversation-first, tranche verticale utile, ordre strict et reprise exacte | Droits du fil ajoutés dans le Conversation Hub et visibles en français; aucun CRM/Kanban/dashboard/OS-6 | Publication et CI complète soumises à l'ordre explicite « publie et lance la CI » |
| 10-12, 17-18, 64-65 | Fil canonique avec droits explicites, policy avant IO et actions sûres | Confidentialité et visibilité durables restituées; policy des pièces jointes reçoit fil/niveau/portée à prepare/read; aucun provider appelé | Autorisations durables nécessaires pour appliquer personal/team/case; Graph et stockage réel non configurés |
| 22-24, 35-38 | Tenant/RLS, classification obligatoire, droits hérités et fichier externe non fiable | Migration 111/0105, défauts sûrs, contraintes, index tenant-leading, jointure tenant-scoped et revalidation des droits après IO | Test PostgreSQL/RLS à exécuter en CI; stockage Supabase/ACL, antivirus et OCR/transcription réels non livrés |
| 32, 69 | DoD et matrice unit/intégration/RLS/provider/workflow/Playwright/sécurité | Ciblés 5 fichiers/21 tests; régression 19/151; suite exhaustive 144/715; ESLint, TypeScript, build et diff check verts | 9 fichiers/22 tests PostgreSQL ignorés sans `DATABASE_URL`; Playwright adapté mais non exécuté; CI requise |

Le PDF canonique est conforme : 71 pages, SHA-256 `bb838fb02c23247b1bcda8981539eebe73264a5334bfaf565aafa5bc26c50fe5`.

## Classification actuelle

- Livré localement : worker média, migrations, rendu Conversation, composition générique, scan, extraction `external_untrusted_data`, contrôle d'intégrité, vue `data-only` non raccordée, accès court, frontière HTTP/session et classification confidentialité/visibilité héritée par la policy, uniquement avec doubles mock.
- Réel connecté : aucun nouveau fournisseur; Graph et stockage réel non composés.
- Sandbox : aucune appelée.
- Mock : fetch, scanner propre/dangereux/temporaire, extracteur, stockage média, codec et lecture HTTP uniquement via doubles injectés; la classification du fil est réelle en base locale mais aucun fournisseur n'en dépend.
