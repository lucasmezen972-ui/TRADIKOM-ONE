...
## Checkpoint applicatif — 4 septembre 2026, 17:49 UTC

- Branche : `codex/tradikom-one-os`; head distant publié `28efa750935b2766de3410b8d9c0d5e3c4e2dbe8`; accès sécurisé conservé localement dans `734c8402de5183b840901bd978777d3fb9d16a84`. Après le handoff documentaire, la branche sera en avance de quatorze commits, sans push; `tmp/` reste non suivi et hors index.
- Travail effectué : ticket AES-256-GCM opaque de 30 à 300 secondes lié par AAD au tenant, à l'utilisateur et à la pièce; policy avant émission et lecture; revalidation membership/métadonnées avant et après IO; contrôle taille/SHA-256; idempotence stable; erreurs et audit sûrs.
- Impact north star : une pièce jointe du fil canonique dispose maintenant d'une frontière de lecture courte, réévaluable et traçable. L'application annonce honnêtement « Téléchargement non configuré » tant qu'aucun stockage n'est injecté et ne présente jamais une référence technique comme une URL utilisable.
- Risques contenus : `disabled` et `not_configured` ne font aucun IO; seul un double `mock` peut lire. Ticket, contenu, nom, checksum et référence restent hors audit. Une altération, expiration, autre acteur/tenant, mutation concurrente ou intégrité invalide échoue fermé.
...
| --- | --- | --- | --- |
| 3-7, 31-33, 46, 48, 70-71 | Conversation-first, tranche verticale utile, ordre strict et reprise exacte | Capacité d'accès ajoutée au Conversation Hub et à son runtime, état français visible, quatre documents actualisés; aucun CRM/Kanban/dashboard/OS-6 | Publication et CI complète soumises à l'ordre explicite « publie et lance la CI » |
| 10-18, 64-65 | Hub canonique, frontière provider, policy, idempotence, activité durable et états honnêtes | Runtime injecté `disabled`/`not_configured`/`mock`; policy avant émission/lecture; même ticket → même clé d'idempotence; erreurs provider réduites à une liste sûre; application `not_configured` | Frontière HTTP/session à ajouter; provider Graph et stockage réels non autorisés/non configurés |
| 22-24, 35-38 | Tenant/RLS, fichier externe non fiable, accès court, hachage, minimisation et défense contre la substitution | AES-256-GCM avec AAD tenant/utilisateur/pièce; 30-300 s; revalidation après IO; taille et SHA-256 recalculés; ticket/nom/contenu/hash/référence hors audit | PostgreSQL/RLS à exécuter en CI; stockage Supabase/ACL, antivirus et OCR/transcription réels non livrés |
| 32, 69 | DoD et matrice unit/intégration/RLS/provider/workflow/Playwright/sécurité | 5 fichiers/50 tests ciblés verts dont 3 PostgreSQL ignorés; régression exhaustive 141 fichiers/693 tests verts; ESLint, TypeScript, build, continuity-check direct et diff check verts | 8 fichiers/21 tests PostgreSQL ignorés sans `DATABASE_URL`; Playwright bloqué avant Conversation par PGlite non partagé; CI requise |

Le PDF canonique est conforme : 71 pages, SHA-256 `bb838fb02c23247b1bcda8981539eebe73264a5334bfaf565aafa5bc26c50fe5`.

## Classification actuelle

- Livré localement : worker média, migrations, rendu Conversation, composition générique, scan, extraction `external_untrusted_data`, contrôle d'intégrité, vue `data-only` non raccordée et accès court chiffré aux pièces jointes, uniquement avec doubles mock.
- Réel connecté : aucun nouveau fournisseur; Graph et stockage réel non composés.
- Sandbox : aucune appelée.
- Mock : fetch, scanner propre/dangereux/temporaire, extracteur, stockage média et codec d'accès uniquement via doubles injectés; affichages explicites « Stockage mock », intégrité vérifiée/extraction masquée et téléchargement non configuré.
