...
## Checkpoint applicatif — 4 septembre 2026, 21:47 UTC

- Branche : `codex/tradikom-one-os`; head distant publié `28efa750935b2766de3410b8d9c0d5e3c4e2dbe8`; propagation des droits aux plans et missions conservée localement dans `754316a5466ca16b730ef0e521852dcce3459b7e`. Après le handoff documentaire, la branche sera en avance de vingt-trois commits, sans push; `tmp/` reste non suivi et hors index.
- Travail effectué : contrôle de l'accès au fil avant génération et avant persistance d'un plan; garde non énumérable sur lecture, liste, décision et exécution; seconde garde transactionnelle lors d'une reprise; filtrage de la liste générique des missions et protection de ses commandes annuler/approuver/refuser/relancer.
- Impact north star : une mission issue d'une conversation hérite désormais des mêmes droits que son fil, y compris après révocation, sans route générique permettant de contourner le Conversation Hub.
- Risques contenus : la révocation concurrente testée entre deux transactions ne modifie ni état, ni compteur, ni audit; la finalisation système d'une mission déjà créée reste bornée et idempotente; aucun contenu sensible ou bénéficiaire n'entre dans l'audit.
...
| --- | --- | --- | --- |
| 3-7, 31-33, 46, 48, 70-71 | Conversation-first, tranche verticale utile, ordre strict et reprise exacte | Droits du fil propagés aux plans et missions; aucun CRM/Kanban/dashboard/OS-6 | Publication et CI complète soumises à l'ordre explicite « publie et lance la CI » |
| 10-12, 15-18, 24, 46, 64, 68 | Un plan validé doit respecter les permissions; seule une mission durable, idempotente et observable peut être lancée | Garde avant génération/persistance puis sur lecture, liste, décision, exécution et reprise; liste et commandes génériques protégées; finalisation système bornée conservée | Étendre la frontière aux événements actifs, à la file et aux incidents/dead letters d'un plan inaccessible |
| 22, 28, 35-38, 43-44 | Isolation tenant, RLS, non-énumération, audit minimisé et mutation atomique | Recherche par tenant et droit actif; accès refusé comme ressource absente; révocation injectée entre transactions avec zéro reprise, zéro compteur et zéro audit | PostgreSQL/RLS réel à exécuter en CI; stockage Supabase/ACL, antivirus et OCR/transcription réels non livrés |
| 32, 69 | DoD et matrice unit/intégration/RLS/provider/workflow/Playwright/sécurité | Ciblés : 16 tests verts; régression : 53 verts et 1 PostgreSQL ignoré; exhaustive : 724 verts et 23 ignorés; ESLint, TypeScript, build, continuity-check et diff check verts | 23 tests PostgreSQL ignorés sans `DATABASE_URL`; Playwright et CI du head requis |

Le PDF canonique est conforme : 71 pages, SHA-256 `bb838fb02c23247b1bcda8981539eebe73264a5334bfaf565aafa5bc26c50fe5`.

## Classification actuelle

- Livré localement : worker média, migrations, rendu Conversation, composition générique, scan, extraction `external_untrusted_data`, contrôle d'intégrité, vue `data-only` non raccordée, accès court, frontière HTTP/session, classification confidentialité/visibilité et autorisations durables personal/team/case appliquées aux fils, messages, pièces jointes, plans et missions; doubles mock uniquement pour les fournisseurs.
...
- Livré localement, CI non déclenchée : worker média durable et composition générique, validation binaire/hachage, scanner puis extraction `external_untrusted_data` obligatoires, vérification SHA-256 à la lecture, vue `data-only` non raccordée, stockage/pièce jointe mock idempotents, accès court et frontière HTTP/session, classification durable et autorisations utilisateur personal/team/case appliquées aux fils/messages/pièces jointes ainsi qu'aux plans et missions, avec interface visible en français.
