...
## Checkpoint applicatif — 4 septembre 2026, 22:48 UTC

- Branche : `codex/tradikom-one-os`; head distant publié `28efa750935b2766de3410b8d9c0d5e3c4e2dbe8`; protection des événements et incidents Conversation conservée localement dans `517a8a73acf9f4f07afe254dd84c8489e2f6e37a`. Après le handoff documentaire, la branche sera en avance de vingt-cinq commits, sans push; `tmp/` reste non suivi et hors index.
- Travail effectué : résolution interne du plan ou de la mission par `planId`/`runId`; filtrage paginé des événements actifs et dead letters selon le droit du fil; garde transactionnelle sur annulation et relance; payload jamais rendu; événements génériques compatibles.
- Impact north star : le « travail en cours » et ses incidents respectent maintenant la même frontière que la conversation qui les a produits, sans laisser une route technique révéler ou relancer une mission révoquée.
- Risques contenus : le refus est non énumérable et ne modifie ni statut, ni tentatives, ni audit; treize reprises masquées n'évincent pas une ligne générique visible; seuls des agrégats de statut sans identifiant restent tenant-wide.
...
| --- | --- | --- | --- |
| 3-7, 31-33, 46, 48, 70-71 | Conversation-first, tranche verticale utile, ordre strict et reprise exacte | File et incidents rattachés à leur conversation; aucun CRM/Kanban/dashboard/OS-6 | Publication et CI complète soumises à l'ordre explicite « publie et lance la CI » |
| 10-12, 15-18, 24, 28, 46, 64, 68 | Les preuves et événements d'une mission durable suivent ses permissions et restent observables sans contenu sensible | Résolution `planId`/`runId`, listes actives/dead-letter filtrées, payload non rendu, génériques compatibles, agrégats non identifiants | Porter la même frontière au niveau RLS PostgreSQL des objets dérivés |
| 22, 35-38, 43-44 | Isolation tenant/RLS, refus non énumérable, audit minimisé et mutation atomique | Annulation/retry revalidés sous transaction; statut, tentatives et audit inchangés après révocation; pagination bornée sans éviction | RLS intra-tenant de plans, missions et événements encore tenant-scoped; migration additive et tests restricted-role requis |
| 32, 69 | DoD et matrice unit/intégration/RLS/provider/workflow/Playwright/sécurité | Ciblés : 27 tests verts; régression : 96 verts et 2 PostgreSQL ignorés; exhaustive : 724 verts et 23 ignorés; ESLint, TypeScript, build, continuity-check et diff check verts | 23 tests PostgreSQL ignorés sans `DATABASE_URL`; Playwright et CI du head requis |

Le PDF canonique est conforme : 71 pages, SHA-256 `bb838fb02c23247b1bcda8981539eebe73264a5334bfaf565aafa5bc26c50fe5`.

## Classification actuelle

- Livré localement : worker média, migrations, rendu Conversation, composition générique, scan, extraction `external_untrusted_data`, contrôle d'intégrité, vue `data-only` non raccordée, accès court, frontière HTTP/session, classification confidentialité/visibilité et autorisations durables personal/team/case appliquées aux fils, messages, pièces jointes, plans, missions, événements actifs et incidents; doubles mock uniquement pour les fournisseurs.
...
Le coffre Meta, l'enveloppe officielle, les statuts, leurs lots, les lots entrants, l'enveloppe mixte et la représentation média sont publiés et prouvés avec PostgreSQL/RLS. La réservation durable est publiée mais sa CI reste bloquée avant tests par l'API d'audit npm. Le worker durable, sa composition générique, le stockage mock, le scanner obligatoire, l'extraction `external_untrusted_data`, le contrôle d'intégrité, la vue `data-only`, l'accès court, sa frontière HTTP/session, la classification confidentialité/visibilité et les autorisations durables personal/team/case jusqu'aux événements actifs et incidents sont livrés localement seulement : ils ne deviennent prouvés PostgreSQL/RLS et Playwright qu'après l'ordre explicite « publie et lance la CI » et une CI complète, sans contourner l'audit. La prochaine tranche locale revient à l'étape RLS de l'ordre directeur : les politiques PostgreSQL des fils, messages, pièces jointes, plans, étapes, missions et événements dérivés restent tenant-scoped et doivent aussi appliquer `app.actor_id` et le droit actif, avec bypass système borné et tests restricted-role; `goal-watch`/OS-6 reste hors périmètre. Les écarts réels restent stockage Supabase/ACL, antivirus, OCR/transcription et Graph. Le premier écart fournisseur demeure Meta for Developers : l'utilisateur saisit le code SMS directement sans le transmettre au chat. Une fois l'inscription validée, inventorier app/WABA/Phone Number ID, puis demander une confirmation immédiatement avant toute création de token persistant. Aucune requête Graph, stockage réel, message, activation, fusion, déploiement ou dépense n'est autorisée par ce checkpoint.
