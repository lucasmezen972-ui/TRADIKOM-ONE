...
- Les pages cœur 3-7, 31-33, 46, 48 et 69-71 et les pages candidates OS-5 10-18, 22-24, 29-30 et 64-68 ont été relues directement le 9 septembre 2026. L'ordre page 48 confirme que la tranche Meta reste conversation-first; la Definition of Done page 32 et la matrice page 69 exigent une preuve PostgreSQL/RLS, build et Playwright en CI.
- La tranche locale courante lie désormais la rotation d'un secret Meta à l'endpoint tenant immuable : WABA et Phone Number ID sont comparés à leurs empreintes HMAC avec la même clé, en temps constant, avant toute mutation ou écriture d'audit. Une clé erronée, une destination différente ou un endpoint incompatible échoue fermé.
- Conversation permet maintenant aux seuls propriétaire et administrateur d'autoriser puis révoquer un essai d'un message. L'émission résout exactement un endpoint Meta configuré, refuse zéro ou plusieurs endpoints, rejoue la même clé sans doublon et interdit une nouvelle autorisation après consommation. La révocation verrouille tous les endpoints Meta et reste efficace même si un secret a été retiré ou l'endpoint désactivé.
- L'état visible est calculé depuis la base : requis, valide, épuisé ou configuration ambiguë. Les confirmations françaises ne dépendent pas d'un paramètre forgé, sont annoncées comme état dynamique et n'exposent ni identifiant fournisseur, ni secret, ni action Graph.
- Preuves locales de la tranche courante : 62 tests ciblés verts; suite exhaustive 152 fichiers/809 tests verts, avec 11 fichiers/27 tests PostgreSQL ignorés faute de `DATABASE_URL`; ESLint, TypeScript, audit high et diff check verts. Le build refuse correctement les trois variables de production absentes; le Playwright local reste non concluant à cause des processus PGlite séparés. La CI PostgreSQL est donc requise avant de déclarer la tranche prouvée.
...
1. Publier strictement en fast-forward la tranche locale endpoint-secret et commandes administratives Conversation, sans `tmp/`, puis suivre CI et continuité jusqu'au résultat final; corriger tout échec réel avant une autre tranche.
2. Une fois la preuve CI verte, l'utilisateur saisit lui-même le code SMS dans Meta for Developers, sans jamais le transmettre dans le chat, puis indique seulement que cette étape est terminée.
3. Inventorier en lecture seule l'application, le WABA et le Phone Number ID dans la console officielle, sans copier de secret dans le chat, les logs ou Git.
4. Demander une confirmation au moment exact avant tout token persistant; exiger ensuite une autorisation distincte avant Graph, message d'essai, webhook public, fusion, déploiement ou dépense. Continuer les travaux OS-5 non bloqués seulement s'ils respectent l'ordre page 48.

## Tranche applicative locale en attente de CI : cohérence endpoint-secret et contrôle d'essai depuis Conversation
...
- État honnête : livré localement et testé sans fournisseur; réel connecté = aucun; sandbox = aucune; mock = fixtures/doubles uniquement; bloqué humain = SMS Meta et token; hors périmètre = Graph, message externe, endpoint public, fusion, déploiement, dépense, CRM, Kanban, dashboard secondaire et OS-6.
