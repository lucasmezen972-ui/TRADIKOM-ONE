...
1. Compléter le même jalon de la page 48 par un reçu de policy serveur append-only, immuable et versionné, lié au tenant, au plan/fingerprint, à la validation, au catalogue de capacités, au fournisseur canonique `tradikom_mock`, aux scopes, au rôle et au risque. L'émettre atomiquement avec l'approbation, ou avec la création pour `approval:none`; un rejet n'en reçoit jamais.
2. Fermer `providerPreference` par une allowlist serveur et recalculer cette décision avant l'exécution, tout appel direct au moteur, chaque reprise durable et la finalisation. Toute dérive de plan, policy, catalogue, fournisseur, rôle, membership ou scope doit échouer avant provider ou nouvel effet métier; les plans historiques approuvés sans reçu restent fermés.
3. Prouver les refus adversariaux : JSON ou empreinte falsifiés, plan A utilisé pour plan B, cross-tenant, fournisseur inconnu ou multiple, scope retiré, rôle révoqué, version de catalogue changée, appel direct sans reçu, dérive avant reprise et mission réussie forgée; conserver le parcours `tradikom_mock` valide, la reprise et le rejeu idempotent sans `fetch`.
