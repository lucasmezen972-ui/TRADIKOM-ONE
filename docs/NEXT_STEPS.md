...
## Situation actuelle — 14 septembre 2026, 23:25 UTC
...
- La continuité `34922145838` est verte. La CI `34922145782` a validé migrations, sauvegarde/restauration, lint et TypeScript, puis a trouvé deux régressions RLS PostgreSQL sur 928 tests : lecture d'un message interne par un collaborateur et reprise bloquée d'une livraison ordinaire sans contexte acteur. La migration additive 0115 ferme la première et corrige la seconde sans modifier la protection d'insertion ni l'isolation cross-tenant. Elle calcule et fige une classification interne depuis le message ou l'identité, y compris lors de la mise à niveau 120 vers 121. Les 17 fichiers de migration et 66 tests passent; le ciblage RLS passe 1 fichier/12 tests et ignore 2 fichiers/3 tests PostgreSQL faute de `DATABASE_URL`. Une revue PostgreSQL éphémère couvre les deux orientations adversariales et ne trouve aucun P1/P2. ESLint, TypeScript, build factice, audit high, continuity-check et diff check sont verts.
- État honnête : livré et publié = révision immuable; livré localement = correctif RLS 0115 en attente de publication et CI; réel connecté = aucun; sandbox = aucune; mock = `tradikom_mock`; bloqué humain = SMS Meta et autorisation distincte au moment exact avant Graph; hors périmètre = Graph, message réel, fusion, déploiement, dépense, CRM, Kanban, dashboard secondaire et OS-6.

## Prochaine action exacte — 14 septembre 2026, 23:25 UTC

1. Publier la migration corrective 0115, ses miroirs, tests et documents suivis en fast-forward sur `codex/tradikom-one-os`, sans jamais indexer `tmp/`.
2. Attendre la CI autoritative et exiger les deux scénarios PostgreSQL corrigés ainsi que migrations, `db:verify`, sauvegarde/restauration, lint, TypeScript, 928/928 tests, build et 20/20 Playwright verts; corriger tout échec avant de poursuivre.
...
- Les pages cœur 3-7, 31-33, 46, 48 et 69-71 et les pages candidates OS-5 15-18, 35-38, 43-44 et 64-68 ont été relues directement le 11 septembre 2026. L'ordre page 48 reste conversation-first; la Definition of Done page 32 et la matrice page 69 exigent notamment les preuves PostgreSQL/RLS et Playwright.
- Le commit applicatif local `9879cae7e8bd0c9e598cd1c35ac55089f7a29b21` livre le reçu de policy serveur OS-5 : une décision canonique, versionnée, profondément immuable et append-only lie tenant, plan/fingerprint, validation, catalogue, unique fournisseur `tradikom_mock`, scopes, rôle et risque. Le reçu est émis atomiquement à l'approbation, ou dès la création pour `approval:none`; un rejet n'en reçoit jamais.
- Les appels directs, l'exécution, les reprises worker, les relances manuelles et la finalisation recalculent désormais la définition et la policy exactes. Une dérive de plan, étape, validation, catalogue, fournisseur, rôle, membership, scope, événement, mission ou résultat échoue avant provider et nouvel effet métier. Les plans historiques approuvés sans reçu restent fermés.
- Les écritures Conversation sensibles sur plans, étapes, validations, missions et événements sont réservées au runtime système par des policies RLS restrictives. Les reçus sont immuables, les suppressions indirectes de plan sont fermées tant que le tenant existe, et la suppression explicite d'un tenant reste possible sans orphelin. Les effets du handler et le succès de l'événement sont atomiques; après épuisement d'une reprise, mission échouée, preuve terminale et lettre morte sont persistées ensemble puis la relance manuelle reste possible.
- Preuves locales finales : 15 fichiers ciblés, 86 tests verts et 7 tests PostgreSQL ignorés faute de `DATABASE_URL`; suite exhaustive 159 fichiers/874 tests verts et 13 fichiers/31 tests ignorés, soit 905 tests sans échec. ESLint, TypeScript, build production factice, continuity-check, diff check et audit high sont verts; l'audit signale trois avis modérés (`qs`, `csv-parse`) et zéro high/critical. La CI PostgreSQL/RLS et les 20 Playwright du futur head restent requis avant de classer cette tranche « prouvée CI ».
- Le lot `9879cae` et son checkpoint `26cc3f1` sont publiés. La continuité `34573066273` est verte. La CI `34573066265` a validé audit, migrations, `db:verify`, sauvegarde/restauration, lint, TypeScript, toute la suite PostgreSQL/RLS et le build de production; 19 Playwright sur 20 ont réussi. Le seul échec comparait l'ancien texte de résultat à la nouvelle preuve générique réellement affichée. Le correctif ciblé `e704afd` aligne l'assertion sans modifier l'application; ESLint ciblé, TypeScript et diff check sont verts.
- La validation Conversation est désormais stricte et publiée dans `dc9cf0c641bab56ac66d451aac5b18368ef8c12a` : une valeur autre que `approved` ou `rejected` échoue avant service, les redirections utilisent les identifiants autoritatifs et une confirmation visuelle n'apparaît que si l'identifiant et l'état durable du plan concordent. Un refus initial annule les deux étapes, bloque la mission et laisse zéro workflow, événement d'exécution, résultat, tâche ou appel provider/fetch observé; l'audit de refus contient uniquement quatre champs autorisés et aucun motif métier.
- Preuves locales sur `dc9cf0c` : 3 fichiers/27 tests ciblés, puis 165 fichiers/849 tests verts et 11 fichiers/29 tests ignorés sans PostgreSQL local, soit 878 tests au total; canari provider ciblé vert après durcissement final. ESLint, TypeScript, build production avec valeurs factices, audit high, continuity-check et diff check sont verts. La continuité distante `34558477110` est verte. La CI `34558477111` a validé audit, migrations, sauvegarde/restauration, lint, TypeScript, les 165 fichiers/878 tests PostgreSQL et le build, puis a échoué uniquement parce que le sélecteur Playwright omettait le préfixe visible « 1. ». Le correctif `84a52a2bebf51c546e1f9f27dbf98dcb79dade20` cible les deux libellés numérotés. La continuité `34560151763` est verte et la CI `34560151750` est entièrement verte sur ce même head : audit, migrations, `db:verify`, sauvegarde/restauration, lint, TypeScript, 165 fichiers/878 tests PostgreSQL, build production et 20/20 Playwright.
...
- La frontière mock/HTTP est structurelle : un manifeste `mock` n'accepte qu'un transport mock et le transport HTTP n'accepte que `ready`. Conversation affiche en français `requise`, `valide` ou `épuisée`, sans bouton d'activation/envoi, identifiant, date ou secret.
- Une consommation déjà enregistrée ferme désormais toute reprise en résultat de transport incertain, sans second appel HTTP, y compris si l'autorisation expire ou est révoquée entre-temps. Une autorisation non encore consommée reste refusée si elle est absente, expirée, révoquée, étrangère ou épuisée.
...
1. Publier strictement en fast-forward `e704afd` et ce checkpoint documentaire sur `codex/tradikom-one-os`, sans inclure `tmp/`.
2. Attendre la CI autoritative et exiger 20/20 Playwright; les migrations, PostgreSQL/RLS, la suite exhaustive et le build ont déjà réussi dans `34573066265`. Corriger tout nouvel échec réel avant de revendiquer la tranche comme prouvée CI.
3. Une fois cette preuve verte, relire la page 48 et reprendre sa première lacune applicative encore non terminée dans le parcours Conversation. Ne pas sélectionner CRM, Kanban, dashboard secondaire ou OS-6. Si toutes les étapes non externes de la page 48 sont closes, conserver le bloc de reprise Meta ci-dessous et poursuivre seulement un travail OS-5 explicitement prouvé et non ambigu.

### Checkpoint humain parallèle — bloqué

- L'utilisateur saisit lui-même le code SMS dans Meta for Developers sans jamais le transmettre dans le chat, puis indique seulement que cette étape est terminée. L'inventaire officiel reste en lecture seule; toute création de token persistant, requête Graph, activation, message d'essai, webhook public, fusion, déploiement ou dépense exige une confirmation distincte au moment exact.

## Tranche applicative publiée et prouvée CI : validation unique stricte et confirmation d'état durable

- Le formulaire de décision est validé par une enum fermée à l'intérieur de la frontière d'erreur publique. Une valeur malformée produit une erreur française sûre, sans service, revalidation, redirection, audit ou message de décision.
- Création, approbation, refus et exécution redirigent vers le fil et le plan réellement retournés par le service. La confirmation visuelle compare l'identifiant demandé à un plan accessible du fil tenant puis exige l'état durable correspondant; les états créé, approuvé et refusé exigent l'absence de mission, tandis que l'exécution exige une mission réussie.
- Cette confirmation atteste honnêtement l'état courant mais reste rejouable par URL; elle ne prouve pas la causalité de la navigation et ne sert jamais d'autorisation.
- Le refus initial est prouvé par le service et dans le parcours desktop/mobile : validation rejetée, deux étapes annulées, exécution ultérieure refusée, zéro workflow, événement d'exécution, résultat ou tâche, canaris provider et `fetch` non appelés. L'audit est comparé à une allowlist JSON exacte sans motif métier.
- Conversation affiche les deux capacités avec des libellés français, masque leurs codes internes, retire les actions Exécuter/Reprendre après refus et ne déborde pas horizontalement. Aucun réseau Meta n'est observé.
- État : livré, publié strictement en fast-forward dans `dc9cf0c`, puis sélecteurs corrigés dans `84a52a2`, et prouvé par la CI autoritative `34560151750` ainsi que la continuité `34560151763`. Le premier run `34558477111` a validé tous les contrôles jusqu'au build, puis a échoué uniquement sur le sélecteur Playwright corrigé. Réel connecté = aucun; sandbox = aucune; mock = générateur déterministe et capacités `tradikom_mock`; bloqué humain = SMS Meta; hors périmètre = Graph, message externe, endpoint public, fusion, déploiement, dépense, CRM, Kanban, dashboard secondaire et OS-6.
- Écarts restants : reçu de policy serveur append-only et versionné, revalidation avant exécution/appel direct/reprise/finalisation, sélection provider fermée, provider mock privé et profondément figé, trois avis de dépendances modérés et annotation CI non bloquante sur la transition Node.js 20 vers Node.js 24 de `pnpm/action-setup@v4`.
...
- Une extraction média n'entre dans le générateur que si sa relation tenant/fil/message, son accès acteur et son intégrité SHA-256 sont tous encore valides. Le contexte total est limité à dix sources et 16 000 caractères.
- La donnée filtrée reste éphémère et marquée comme externe non fiable. La provenance durable exclut contenu, extracteur et empreinte; une absence de source reste compatible avec les plans version 1 existants.
- Après génération, droits, message, pièces jointes et extractions sont relus sous verrou et comparés au fingerprint initial. Une révocation ou mutation concurrente laisse zéro effet durable.
- Le mapping public réduit ces échecs à des réponses françaises sûres 400/409. L'interface n'affiche que la provenance de confiance et rappelle qu'aucune instruction ni aucun outil n'est autorisé.
- Les tests couvrent idempotence, changement concurrent, révocation, cross-tenant, absence de fuite, verrous PostgreSQL et parcours desktop/mobile avec approbation et exécution mock sans réseau.
- La seconde CI `34432424191` a ensuite rencontré avant les contrôles applicatifs une course de fermeture `pg-pool`/PostgreSQL (`57P01`) pendant le nettoyage d'une base temporaire. Le vérificateur attend maintenant de façon bornée zéro session dans `pg_stat_activity`, puis supprime la base sans `FORCE`; il échoue explicitement si une session persiste au lieu de masquer l'erreur.
- État honnête au head `f692057` : livré, publié et prouvé. La CI `34433025988` a validé audit, migrations et nettoyage sans `57P01`, `db:verify`, sauvegarde/restauration, lint, TypeScript, 163 fichiers/844 tests PostgreSQL, build et 20/20 Playwright; continuité `34433025991` verte. Réel connecté = aucun; sandbox = aucune; mock = générateur/workflow et doubles média de test; bloqué humain = SMS Meta; hors périmètre = fournisseur réel, Graph, stockage réel, OCR/transcription, fusion, déploiement, dépense, CRM, Kanban, dashboard secondaire et OS-6.

## Tranche applicative publiée et prouvée CI : cohérence endpoint-secret et contrôle d'essai depuis Conversation

- L'empreinte de destination est maintenant produite par un module unique lors de l'enregistrement de l'endpoint et lors de la rotation du secret. Le service Meta verrouille l'endpoint, relit ses invariants et refuse tout WABA, Phone Number ID ou clé HMAC incohérent avant base et audit.
- Les actions serveur Conversation ne prennent ni tenant, ni acteur, ni endpoint du navigateur. Elles utilisent la session, exigent une confirmation explicite du plafond gratuit et traduisent les erreurs en réponses françaises 400/403/404/409 sans détail interne.
- L'autorisation courante est idempotente sous concurrence et limitée à un seul endpoint configuré. Le budget verrouille endpoint, livraison puis autorisation; les tests PostgreSQL ajoutés orchestrent une consommation concurrente afin de prouver qu'une nouvelle émission et une révocation ne lisent pas un snapshot périmé.
- La révocation agit sur toutes les preuves Meta valides et non consommées du tenant, même si la configuration courante a changé. Un état multi-endpoint est explicite et fermé pour l'émission; la révocation de sécurité reste disponible.
- Playwright utilise le vrai formulaire Conversation pour autoriser, révoquer puis autoriser de nouveau. Seule la consommation finale est préparée en base pour rendre l'état épuisé; un espion navigateur exige zéro requête Graph.
- Preuve autoritative : commit `cc0bb2b7ff906bba145f26817f43a3ed55b4dd88` publié strictement en fast-forward; CI `34422242316` verte avec 163 fichiers/836 tests PostgreSQL, build et 20 Playwright; continuité `34422242320` verte. La PR #11 reste ouverte, brouillon, fusionnable et `CLEAN`.
- État honnête : livré, publié et prouvé CI sans fournisseur; réel connecté = aucun; sandbox = aucune; mock = fixtures/doubles uniquement; bloqué humain = SMS Meta et token; hors périmètre = Graph, message externe, endpoint public, fusion, déploiement, dépense, CRM, Kanban, dashboard secondaire et OS-6.
...
- Livré, publié et prouvé CI jusqu'au head précédent : réservation et worker d'import média, scan obligatoire mock, extraction `external_untrusted_data` persistée/immuable, contrôle SHA-256 à la lecture, accès court chiffré, frontière HTTP/session, classification confidentialité/visibilité et droits personal/team/case hérités jusque dans la RLS des objets dérivés.
