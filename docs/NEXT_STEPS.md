...
- Le head local et distant observé avant le correctif est `54755dec72a755f13c1b8602b9bb7c4120b91df1`. Le coffre fonctionnel reste prouvé par la CI `33422211572` et la continuité `33422211485` sur `f99b5f3`; la CI du correctif d'enveloppe officielle reste à obtenir après publication.
...
- Une fixture entièrement anonymisée prouve signature, endpoint à 16 chiffres, ingestion, rejeu idempotent et non-fuite du nom, des numéros, du timestamp et du `wamid` dans les audits, identités et liaisons fournisseur.
...
Les pages 13-22, 26-33, 35-38, 46, 48 et 64-69 imposent signature avant parsing, schémas d'entrée bornés, identité opaque tenant-aware, chiffrement des credentials, action durable, idempotence, audit sans contenu sensible et tests provider/sécurité. La Definition of Done de la page 32 et la matrice de la page 69 exigent la CI du correctif puis le checkpoint fournisseur avant de classer OS-5 terminé.

## Prochaine action concrète

1. Réconcilier le head distant, publier le correctif d'enveloppe officielle sans `tmp/` et obtenir CI + continuité vertes sur le nouveau head.
2. L'utilisateur saisit le code SMS directement dans Chrome puis indique seulement que l'étape est terminée; ne jamais transmettre le code dans le chat.
3. Dans la console officielle, inventorier l'application, le WABA et le Phone Number ID. Demander une confirmation au moment exact avant toute création d'un token persistant.
4. Injecter les valeurs réelles par références de gestionnaire de secrets côté serveur, sans les lire, afficher, journaliser ou commiter. Conserver le provider `not_configured`/`awaiting_human_auth` tant que la composition n'est pas complète.
5. Une requête Graph réelle, un webhook public, un message de preuve, une activation, un déploiement ou une dépense nécessitent une autorisation distincte; aucune de ces actions n'est couverte par l'autorisation de stocker les clés.
...
- Correctif enveloppe : 4 fichiers/24 tests ciblés puis 17 fichiers/108 tests de régression verts; ESLint complet, TypeScript, build production, continuity-check et `git diff --check` verts. La CI du futur head reste attendue.
...
- Livré localement, CI attendue : compatibilité de l'enveloppe webhook officielle et clés internes hashées.
...
3. Le coffre Meta est prouvé CI au head f99b5f3; le correctif d'enveloppe officielle est local au parent 54755de, avec 17 fichiers/108 tests, lint, typecheck, build et continuité verts, mais doit encore être publié et prouvé par CI.
4. Après fetch et ascendance sûre, publier sans tmp/ et attendre CI + continuité vertes.
5. L'onglet Meta for Developers attend le code SMS saisi directement par l'utilisateur; ne demander ni afficher le code.
6. Après validation Meta, demander une confirmation au moment exact avant la création d'un token persistant et stocker les valeurs uniquement via références serveur.
7. Ne déclencher ni Graph, message, endpoint public, déploiement, fusion ou dépense sans autorisation distincte.
