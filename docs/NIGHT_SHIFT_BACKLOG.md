...
4. Validate scoped E2E fix `6a5b589`; `a3ae131` repaired the OAuth origin and run `29377818420` passed the OAuth flow before finding an unrelated ambiguous legacy selector.
5. Validate controlled installed-disabled/read-only connector execution, policy, quota, idempotence, health, worker, RLS and expanded E2E through CI.
6. Implement universal import/export and website domain binding.
