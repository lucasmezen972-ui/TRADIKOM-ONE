...
- `awaiting_human_auth` : les métadonnées techniques sont prêtes, mais le checkpoint humain borné manque, est invalide ou a expiré.
...
La readiness n'appelle jamais Twilio, ne résout aucune référence du gestionnaire de secrets, ne construit aucun keyring et ne retourne ni référence, Account SID, numéro, token ou URL complète.
...
5. Produire, via un futur service administrateur audité et jamais depuis une entrée navigateur brute, une autorisation liée au tenant avec portée `twilio_whatsapp_sandbox`, expiration explicite, confirmation des unités gratuites et plafond d'un ou deux messages.

Sans ces cinq validations, l'état doit rester `awaiting_human_auth` ou inférieur.

## Activation et test borné

1. Vérifier que le feature flag, les références versionnées du keyring, l'endpoint actif du bon tenant et les deux URLs HTTPS passent la readiness.
...
2. Révocation : révoquer les secrets endpoint et identité, puis confirmer que les résolveurs échouent fermé et que la santé redescend à `not_configured` ou `degraded`.
