...
- Added injected-client-aware tenant transactions with transaction-local PostgreSQL context and rollback coverage for tenant/default provisioning, onboarding/Business Twin/site generation, website publication/restoration, invitation acceptance, and public lead CRM/form/audit/domain-event writes.
...
- CSV finalization, accepted webhook effects, and selected workflow transitions still need final transaction coverage.
