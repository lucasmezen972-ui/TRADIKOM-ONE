...
- The worker is now available as both a durable batch dispatcher and a polling process, but still needs persisted action attempts with richer retry/backoff metadata, domain-specific async handlers beyond the lead workflow, and manual recovery controls for failed domain events.
