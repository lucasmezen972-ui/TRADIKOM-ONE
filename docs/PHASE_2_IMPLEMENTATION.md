...
- The worker is now available as both a durable batch dispatcher and a polling process, but still needs richer retry/backoff scheduling metadata, domain-specific async handlers beyond the lead workflow, and manual recovery controls for failed domain events.
