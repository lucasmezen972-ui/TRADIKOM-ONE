...
Statut: première tranche verticale validée sur `codex/phase-5-real-connectivity`.
...
Le head `444d506313067604372641d630a0244f50941564` a passé le run GitHub Actions complet `29512987962`: audit des dépendances, migrations, sauvegarde/restauration, lint, typecheck, tests unitaires et d'intégration, build de production et tous les scénarios Playwright.

La fermeture a également corrigé les soumissions HTML natives de connexion et de catalogues: l'origine est comparée à `APP_URL` et les redirections `303` restent relatives au même site. La CSP `form-action 'self'` n'a pas été assouplie.
...
Les fournisseurs réels restent un lot Phase 5 séparé, soumis à documentation officielle, sandbox contrôlée, credentials dédiés et validation explicite. La Phase 6 reste verrouillée jusqu'à la stabilisation de ce lot suivant.
