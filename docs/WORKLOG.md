...
## 2026-08-31 - Continuité Meta confirmée à 14:21 UTC

- La copie active reste exclusivement `/Users/TRADIKOM/Developer/TRADIKOM-ONE`, lisible, inscriptible et synchronisée au head `6c2d780ee8bbd29342b66faaaf581cd1d9809fcf` sur `codex/tradikom-one-os`; `tmp/` demeure non suivi et préservé.
- Le PDF canonique conserve 71 pages et le SHA-256 `bb838fb02c23247b1bcda8981539eebe73264a5334bfaf565aafa5bc26c50fe5`. Les pages cœur 3-7, 31-33, 46, 48 et 69-71 ainsi que les pages OS-5 13-18, 22, 26-30, 35-38 et 64-68 ont été relues directement; les pages 29, 32, 48, 64, 69 et 71 ont été inspectées en rendu.
...
## 2026-08-31 - Coffre chiffré WhatsApp Meta livré localement à 17:50 UTC

- L'utilisateur a autorisé la configuration des clés Meta. Cette autorisation n'autorise ni message, requête Graph, déploiement, endpoint public, dépense ou fusion, et ne permet pas de faire transiter un secret ou un code SMS dans le chat, les logs, Git ou le modèle.
- L'inscription officielle Meta for Developers est ouverte dans Chrome. Elle reste sur l'étape de vérification du compte : un code SMS à six chiffres doit être saisi directement par l'utilisateur; le bouton Continuer est encore désactivé. Aucun code, token, app, WABA ou Phone Number ID n'a été lu ou créé.
- La migration additive runtime `103_os5_channel_provider_secret_versions_meta` et son miroir SQL `0097_os5_channel_provider_secret_versions_meta.sql` étendent le coffre à `whatsapp_meta`. Une clé étrangère composée lie chaque secret au même tenant, endpoint et provider; aucune politique RLS ni permission Data API publique n'est affaiblie.
- Le repository et le service de secrets sont maintenant provider-scoped. Les contrats Twilio restent compatibles; Meta ajoute rotation/révocation endpoint et identité, schémas bornés, exigence de liaison endpoint-identité et résolveurs serveur pour credentials, destination et secrets webhook.
- Les valeurs Meta sont chiffrées en AES-256-GCM avec AAD tenant/provider/endpoint/identité/portée/version. Rejeu identique, collision d'idempotence, rotation de clé, révocation monotone, mauvais WABA, rôle insuffisant, cross-tenant, endpoint désactivé et identité non liée échouent de façon sûre. Les audits n'enregistrent aucune valeur sensible.
- Le transport Meta est branché uniquement avec `fetch` factice et état `mock`. Aucune requête réseau fournisseur ou message externe n'a été déclenché.
- Validation locale : 14 fichiers réussis et 2 fichiers PostgreSQL ignorés; 91 tests réussis et 2 ignorés sans `DATABASE_URL`. Base neuve et upgrade depuis runtime 101 sont prouvées sous PGlite. ESLint complet, TypeScript, diff check et build Next.js production sont verts.
...
## 2026-08-31 - Enveloppe webhook officielle Meta compatible localement à 18:29 UTC

- L'exemple de webhook WhatsApp Cloud transmis par l'utilisateur a révélé un défaut logiciel concret : l'adaptateur strict refusait `messaging_product`, `display_phone_number`, `contacts` et `timestamp`; un vrai `wamid` terminé par `=` produisait en outre des clés internes incompatibles avec les contrats d'idempotence. Aucune valeur de cet exemple n'a été copiée dans le dépôt, les tests, les audits ou la documentation.
- L'adaptateur accepte maintenant explicitement et de façon bornée les champs officiels nécessaires, un Phone Number ID jusqu'à 64 chiffres et un `wamid` jusqu'à 256 caractères avec padding base64. L'identifiant fournisseur reste l'`externalMessageId` opaque de provenance; l'idempotence et la corrélation utilisent une empreinte SHA-256 hexadécimale stable et sûre.
- Une fixture entièrement anonymisée reproduit l'enveloppe complète. La preuve d'intégration couvre signature HMAC, endpoint à 16 chiffres, ingestion conversationnelle, rejeu sans doublon et conservation du `wamid` dans la seule provenance métier requise. Le nom de contact, le numéro affiché, le sender, le Phone Number ID, le timestamp et le `wamid` restent absents des audits, identités et liaisons fournisseur.
...
## 2026-09-01 - Notifications de statut WhatsApp Cloud Meta prêtes localement à 02:00 UTC

- Le parent local et distant `b5a0fe3` reste entièrement vert : CI `33430233674`, continuité `33430233673`, PR #11 ouverte, brouillon et `MERGEABLE/CLEAN`. `tmp/` est resté non suivi et hors index.
- Le PDF canonique a été revérifié à 71 pages avec le SHA-256 exact. Les pages cœur 3-7, 31-33, 46, 48 et 69-71 ainsi que les pages OS-5 13-22, 26-38 et 64-68 ont été relues directement; les pages 48 et 69 ont été inspectées en rendu. L'alignement a été écrit dans `AGENT_STATE` avant le code.
- La documentation officielle Meta confirme les notifications `sent`, `delivered`, `read` et `failed` et précise que leur ordre de réception peut différer de leur chronologie. La tranche accepte aussi `deleted` conformément à la référence webhook générale.
- Le point d'entrée Meta distingue maintenant messages entrants et notifications `statuses`, toujours après signature du corps brut. WABA et Phone Number ID doivent résoudre un endpoint Meta actif; le `wamid` doit correspondre à une livraison du même endpoint et du même provider.
- La migration runtime `104_os5_whatsapp_meta_delivery_events` et son miroir SQL `0098_os5_whatsapp_meta_delivery_events.sql` étendent additivement les événements immuables à Meta, ajoutent la relation composée tenant/livraison/provider et autorisent les `wamid` sortants opaques avec padding base64.
- La réconciliation est monotone et idempotente : replay sans doublon, `delivered` sans régression, convergence `failed` puis `read`, code d'échec interne générique. Timestamp, destinataire, WABA, Phone Number ID, `wamid`, détails d'erreur et payload brut restent hors événements et audits.
- Preuves locales : 4 fichiers/27 tests statut-migrations-HTTP verts; régression Meta 15 fichiers/97 tests verts; ESLint ciblé et complet, TypeScript, build Next.js production, `git diff --check` et continuity-check verts. Le build a nécessité une restauration exacte des 601 dépendances depuis le store/lockfile après une tentative sandbox; le dossier incomplet a été conservé dans `/private/tmp`.
- La CI PostgreSQL/RLS du futur head reste nécessaire pour la Definition of Done. Aucun secret, compte, app, WABA, numéro réel, endpoint public, requête Graph, message externe, dépense, fusion ou déploiement n'a été créé ou déclenché. Le code SMS Meta reste le checkpoint humain après publication et CI.

## 2026-09-01 - Lots de statuts WhatsApp Meta durcis localement à 02:35 UTC

- Le checkpoint précédent `9551f608203db2671fdc5d853a8c2c11231efd2a` est publié et entièrement prouvé : CI `33460999183` verte sur migrations PostgreSQL, backup/restauration, RLS, lint, typecheck, suite exhaustive, build production et Playwright; continuité `33460999408` verte. La PR #11 reste ouverte, brouillon et `MERGEABLE/CLEAN`.
- Le PDF canonique a été revérifié à 71 pages avec le SHA-256 exact. Les pages cœur 3-7, 31-33, 46, 48 et 69-71 ainsi que les pages OS-5 13-22, 26-38 et 64-68 ont été relues directement; les pages 48 et 69 ont été inspectées en rendu. `AGENT_STATE.masterPrompt.alignment` a été actualisé avant le code.
- La documentation officielle Meta représente `entry`, `changes` et `statuses` comme des tableaux. La tranche de robustesse accepte donc plusieurs statuts dans des bornes strictes : dix éléments maximum par niveau et cent événements maximum par requête.
- La signature du corps brut reste vérifiée avant parsing. Toutes les références WABA/Phone Number ID/endpoint/livraison du lot sont prévalidées avant la première mutation; une référence inconnue ultérieure refuse l'ensemble sans événement ni audit partiel.
- Chaque statut conserve sa clé d'événement durable, sa déduplication et sa projection monotone. Le replay complet ne crée aucun doublon; la réponse HTTP reste générique et n'expose aucune référence fournisseur ou PII.
- Un premier test a démontré qu'une écriture pouvait précéder la découverte d'une référence inconnue dans la base de test. La prévalidation en deux passes corrige ce cas; le test prouve désormais zéro événement, zéro audit et état de livraison inchangé.
- Preuves locales actuelles : 2 fichiers/20 tests ciblés verts; régression Meta 14 fichiers/96 tests verts et 1 test PostgreSQL ignoré sans `DATABASE_URL`; ESLint ciblé et complet, TypeScript, build production, continuity-check et `git diff --check` verts. Le build a été relancé hors sandbox uniquement pour les polices Google déjà déclarées. La CI PostgreSQL/RLS du futur head reste à exécuter.
- Aucun secret, code SMS, compte finalisé, app, WABA, numéro réel, endpoint public, requête Graph, message externe, dépense, fusion ou déploiement n'a été créé ou déclenché. `tmp/` reste non suivi, préservé et hors index; le checkpoint humain reste la saisie directe du code SMS dans Meta.

## 2026-09-01 - Lots de messages entrants WhatsApp Meta durcis localement à 05:21 UTC

- Le head local et distant `159dc13c36ff1c203aae03fe023ba1819709b537` est entièrement prouvé : CI `33463499865` verte sur migrations PostgreSQL, backup/restauration, RLS, lint, typecheck, suite exhaustive, build et Playwright; continuité `33463499881` verte. La PR #11 reste ouverte, brouillon et `MERGEABLE/CLEAN`.
- Le PDF canonique a été revérifié à 71 pages avec le SHA-256 exact. Les pages cœur 3-7, 31-33, 46, 48 et 69-71 ainsi que les pages OS-5 10-18, 22, 26-38 et 64-68 ont été relues directement; les pages 48 et 69 ont été inspectées en rendu. `AGENT_STATE.masterPrompt.alignment` a été actualisé avant le code.
- La référence officielle Meta décrit `entry`, `changes` et `messages` comme des tableaux. La tranche accepte désormais dix éléments maximum par niveau et cent messages texte maximum par requête, après vérification HMAC du corps brut et avant tout parsing.
- Tous les couples WABA/Phone Number ID sont prévalidés avant la première mutation. Une référence inconnue en fin de lot refuse l'ensemble sans message, binding ou audit conversationnel partiel. Chaque message garde sa provenance, sa clé SHA-256 et son rejeu idempotent.
- Une fixture de lot prouve trois messages ordonnés et rejoués sans doublon. Une autre enveloppe contenant deux endpoints de deux tenants reste strictement isolée; aucune identité n'est fusionnée et aucune donnée fournisseur n'entre dans l'audit.
- Preuves locales : 2 fichiers/13 tests ciblés verts; régression Meta 13 fichiers réussis, 3 fichiers PostgreSQL ignorés, 96 tests réussis et 3 ignorés faute de `DATABASE_URL`; ESLint ciblé et complet, TypeScript, build Next.js production, continuity-check et `git diff --check` verts. Le build a été relancé hors sandbox uniquement pour les polices Google déjà déclarées.
- La suite exhaustive locale est restée silencieuse plus de trois minutes et a été interrompue sans assertion en échec; elle n'est pas présentée comme verte. La CI du futur head doit prouver PostgreSQL/RLS, suite complète, build et Playwright. Écart explicite restant : une enveloppe mélangeant messages entrants et statuts doit encore être dispatchée dans une tranche ultérieure.
- Aucun secret, code SMS, compte finalisé, app, WABA, numéro réel, endpoint public, requête Graph, message externe, dépense, fusion ou déploiement n'a été créé ou déclenché. `tmp/` reste non suivi, préservé et hors index; le checkpoint humain reste la saisie directe du code SMS dans Meta.

## 2026-09-01 - Enveloppe Meta mixte unifiée localement à 06:02 UTC

- Le head local et distant `4e9f35dcd7ec7a938f1674293e615c79d5e1d8f6` est entièrement prouvé : CI `33473526862` verte sur migrations PostgreSQL, backup/restauration, RLS, lint, typecheck, suite exhaustive, build et Playwright; continuité `33473526866` verte. La PR #11 reste ouverte, brouillon et `MERGEABLE/CLEAN`.
- Le PDF canonique a été revérifié à 71 pages avec le SHA-256 exact. Les pages cœur 3-7, 31-33, 46, 48 et 69-71 ainsi que les pages OS-5 10-18, 22, 26-38 et 64-68 ont été relues directement; les pages 48 et 69 ont été inspectées en rendu. `AGENT_STATE.masterPrompt.alignment` a été actualisé avant le code.
- La référence officielle Meta conserve une enveloppe générique avec tableaux `entry` et `changes`, puis un payload spécifique. La route ne tente plus de valider toute la requête comme statuts avant de retenter toute la requête comme messages : elle vérifie le HMAC une seule fois, parse le JSON ensuite et distribue chaque changement vers son schéma strict.
- Les bornes de dix entrées et dix changements sont conservées; le total combiné messages + statuts est limité à cent. Un changement contenant les deux familles ou aucune est refusé avant base.
- Tous les endpoints des messages et statuts, ainsi que toutes les livraisons sortantes, sont prévalidés avant la première mutation. La persistance des messages, bindings, événements et projections s'effectue ensuite dans une transaction unique.
- Le test dédié prouve une enveloppe mixte, les enveloppes mono-famille, le rejeu complet, la borne globale, le refus d'un changement ambigu, l'absence d'accès base sur signature altérée et l'annulation du message entrant si une livraison du même lot est inconnue. Les audits restent sans contenu, numéros, Phone Number ID ou `wamid`.
- Preuves locales : test dédié 5/5 vert; régression ingress/webhook 6 fichiers/41 tests; régression Meta/coffre 18 fichiers/120 tests verts et 2 fichiers/2 tests PostgreSQL ignorés sans `DATABASE_URL`; ESLint complet, TypeScript, build production, continuity-check et `git diff --check` verts. Le build a été relancé hors sandbox uniquement pour les polices Google déjà déclarées.
- La CI du futur head doit encore prouver PostgreSQL/RLS, suite exhaustive, build et Playwright. Aucun secret, code SMS, compte finalisé, app, WABA, numéro réel, endpoint public, requête Graph, message externe, dépense, fusion ou déploiement n'a été créé ou déclenché. `tmp/` reste non suivi, préservé et hors index.

## 2026-09-02 - Médias entrants WhatsApp Meta représentés sans téléchargement à 02:20 UTC

- Le head local et distant `0e92f38cad78b14ec80bafa9444658b49e8169a2` est entièrement prouvé : CI `33523760105` verte sur migrations PostgreSQL, backup/restauration, RLS, lint, typecheck, 144 fichiers/670 tests, build et 20 Playwright; continuité `33523760887` verte. La PR #11 reste ouverte, brouillon et `MERGEABLE/CLEAN`.
- Le PDF canonique a été revérifié à 71 pages avec le SHA-256 exact. Les pages cœur 3-7, 31-33, 46, 48 et 69-71 ainsi que les pages OS-5 10-14, 22-24, 26-29, 34-38 et 64-68 ont été relues directement; les pages 48 et 69 ont été inspectées en rendu. `AGENT_STATE.masterPrompt.alignment` a été actualisé avant le code.
- La documentation officielle Meta confirme que `image`, `audio`, `document`, `video` et `sticker` utilisent un objet média contenant notamment identifiant, MIME et checksum; le sticker peut indiquer `animated`. Les schémas acceptent uniquement des types documentés, bornent les captions/noms et refusent les champs inattendus.
- Après vérification HMAC, le normaliseur ne propage que le type média générique et la légende utile éventuelle. Le service crée une notice française comme « Document WhatsApp en attente d’import sécurisé. » sans conserver Media ID, checksum, nom, MIME, URL ou binaire.
- Aucun appel `fetch` ni ligne `conversation_message_attachments` n'est produit. La tranche ne prétend pas avoir téléchargé, stocké, analysé, transcrit ou OCRisé le média; ces traitements attendent un flux autorisé avec contrôle taille/type, stockage immuable, ACL, checksum et analyse de sécurité.
...
## 2026-09-03 - Réservation durable des imports média Meta prête localement

- Le heartbeat a repris du travail produit au lieu de se limiter à un contrôle : la première tranche OS-5 non bloquée après la représentation conversationnelle des médias réserve désormais leur futur import de façon durable, sans passer à OS-6, au CRM, au Kanban ou à un dashboard secondaire.
- Les migrations runtime `105_os5_channel_provider_media_imports` et `106_os5_channel_provider_media_imports_rls`, avec leurs miroirs SQL `0099` et `0100`, créent une réservation liée par clés composées au même tenant, endpoint Meta et message canonique. L'index commence par `tenant_id`, l'état est contraint et la politique RLS reprend le contexte applicatif existant.
- La référence fournisseur validée reste dans une structure éphémère non sérialisable jusqu'au service d'ingestion. Elle est alors chiffrée en AES-256-GCM avec données associées tenant/provider/endpoint/message et version de clé; Media ID, MIME déclaré, checksum et nom d'origine ne sont jamais persistés en clair ni placés dans l'audit.
- Les états sont honnêtes : `pending` seulement avec référence chiffrée, `not_configured` sans coffre média, `failed` si la protection échoue. Une empreinte HMAC permet le rejeu exact et refuse une collision sur le même message. La réservation et son audit sûr sont dans la même transaction que le message et le binding.
- Aucun téléchargement Graph, `fetch`, stockage Supabase, binaire, antivirus, transcription, OCR ou ligne `conversation_message_attachments` n'a été ajouté. Le code ne présente donc pas une réservation comme un import terminé.
- Preuves locales : chiffrement/migrations/adaptateur/lot, 4 fichiers réussis, 1 fichier PostgreSQL/RLS ignoré, 17 tests réussis et 1 ignoré; ingestion complète 13/13 verte. La régression Meta élargie produit 19 fichiers et 135 tests réussis, avec deux timeouts d'horloge locale repassés isolément en 1/1 chacun. ESLint complet, TypeScript, build Next.js production et `git diff --check` sont verts.
- `pnpm agent:continuity-check` a été tenté comme demandé; le lanceur pnpm local a voulu réinstaller les dépendances sans réseau ni TTY. L'exécution directe du script versionné a retourné `ready`, zéro erreur et zéro avertissement. La CI du futur head doit encore prouver PostgreSQL/RLS, suite exhaustive et Playwright.
- Le parent local et distant `232f60a4530b2a54dd1aee3ce7be75ecc7d6f45a` est réconcilié avant publication; la PR #11 est ouverte, brouillon et `MERGEABLE/CLEAN`, avec CI `33674098147` et continuité `33674098123` vertes. `tmp/` reste non suivi et hors index.
- Aucun secret, code SMS, compte finalisé, app, WABA, numéro réel, endpoint public, requête Graph, message externe, dépense, fusion ou déploiement n'a été créé ou déclenché. Le checkpoint humain reste la saisie directe du code SMS dans Meta, puis une confirmation distincte au moment exact avant tout token persistant ou effet fournisseur.

## 2026-09-04 - Réservation média publiée, CI bloquée par le registre npm

- Au heartbeat de 09:45 UTC, le PDF canonique est revérifié à 71 pages, non chiffré, avec le SHA-256 exact. Les pages cœur 3-7, 31-33, 46, 48 et 69-71 ainsi que 11, 14, 22-23 et 64-65 sont relues directement; les pages 48 et 69 sont inspectées en rendu.
- Le commit `28efa750935b2766de3410b8d9c0d5e3c4e2dbe8` est publié sur `codex/tradikom-one-os`; le premier push a signalé un verrou concurrent, puis le fetch a confirmé que le ref distant contenait déjà exactement ce commit. Aucun force-push, reset, stash, changement de branche ou écrasement n'a eu lieu; `tmp/` reste non suivi.
- La continuité `33826756891` est verte sur le head exact.
- La CI `33826756939` s'est arrêtée deux fois au contrôle initial `pnpm audit`, avant les migrations et les tests. Les deux journaux montrent uniquement l'expiration de l'appel `registry.npmjs.org/-/npm/v1/security/advisories/bulk` après ses retries; aucun avis de vulnérabilité ni défaut applicatif n'a été produit.
- La même commande d'audit, reproduite localement hors sandbox, rencontre le même timeout du registre. Le contrôle reste fail-closed : aucun contournement, suppression d'audit ou changement de dépendance n'est appliqué sans preuve.
- Le nouveau `pnpm agent:continuity-check` est bloqué par le wrapper local qui tente de réinstaller pnpm sans réseau ni TTY. Le script versionné direct final retourne `ready`, zéro erreur et zéro avertissement; le JSON et `git diff --check` sont verts.
- La tentative de troisième relance CI n'a pas atteint GitHub : le contrôleur d'autorisation de l'environnement a lui-même échoué sur une erreur réseau et interdit tout contournement. Une approbation utilisateur explicite de la relance est désormais nécessaire; aucun déclenchement indirect n'est tenté.
- Les preuves locales de la tranche restent vertes : tests ciblés et ingestion, régression Meta avec les deux timeouts d'horloge rejoués, ESLint complet, TypeScript, build production, continuity-check direct et diff check. PostgreSQL/RLS, suite exhaustive et Playwright ne sont pas revendiqués sur ce head tant qu'une CI complète n'a pas abouti.
- Aucun Graph, secret, message, endpoint public, stockage réel, activation, fusion, déploiement ou dépense n'a été déclenché. La prochaine reprise doit d'abord obtenir une CI complète après rétablissement du registre npm, puis seulement préparer le worker d'import avec doubles mock.

## 2026-09-04 - Import média mock durable et visible dans Conversation à 12:11 UTC

- À la demande explicite de poursuivre le développement applicatif, la première tranche non bloquée suivant la réservation publiée a été réalisée localement. Le PDF maître est resté normatif : SHA-256 exact, 71 pages, pages cœur et pages 11, 14, 16-18, 22-23, 35-38 et 64-65 relues avant le code; `masterPrompt.alignment` a été actualisé avant toute implémentation.
- Les migrations runtime `107_os5_channel_provider_media_import_executions` et `108_os5_channel_provider_media_import_executions_rls`, avec miroirs SQL `0101/0102`, créent une exécution liée au même tenant/import/provider. RLS, index tenant-leading, identité immuable, bail, tentatives bornées, backoff et états `reserved`, `succeeded`, `failed` et `denied` sont contraints en base.
- Le worker vérifie accès tenant, contexte actif et policy avant déchiffrement. Les deux frontières provider/storage doivent annoncer `mock`; `disabled` et `not_configured` restent fail-closed sans appel. La limite maximale est transmise au provider puis revérifiée avec type métier, MIME, signature binaire, taille et SHA-256.
- Le stockage mock reçoit une clé d'idempotence stable. Une réussite crée une seule pièce jointe canonique dans la transaction finale; le rejeu ne refait ni lecture, ni stockage, ni insertion. Un échec temporaire est repris sans doubler le stockage; les échecs définitifs, validation, policy et non-configuration restent classés et audités sans contenu sensible.
- L'écran `/conversation` affiche maintenant la provenance WhatsApp au lieu de « Canal de test » et rend nom, taille et badge français « Stockage mock ». Le checksum et la référence technique ne sont jamais affichés.
- Preuves locales : 24 tests ciblés média/migrations verts; 2 tests PostgreSQL/RLS ajoutés et ignorés faute de `DATABASE_URL`; régression exhaustive 139 fichiers/672 tests verts, 8 fichiers/20 tests PostgreSQL ignorés; ESLint complet, TypeScript et build Next.js production verts. Le build a utilisé le réseau uniquement pour les polices Google déjà déclarées. Un scénario Playwright desktop/mobile prouve le rendu mais n'est pas revendiqué localement faute de PostgreSQL partagé.
- GitHub a été relu sans mutation : PR #11 ouverte, brouillon, `MERGEABLE/UNSTABLE` sur `28efa75`; continuité `33826756891` verte et CI `33826756939` inchangée, arrêtée avant tests par le timeout npm audit. Aucun rerun, push ou déclenchement indirect n'a été effectué sans approbation explicite.
- État de vérité : tranche livrée localement, fournisseurs mock uniquement. Aucun Graph, stockage Supabase, antivirus, OCR/transcription, secret, message externe, endpoint public, activation, fusion, déploiement ou dépense. `tmp/` reste non suivi, préservé et hors index.
- Le checkpoint applicatif a été conservé dans le commit local `279b061ec9609dbbf3e5d9aaf21ccd30c9c68052` avec quinze fichiers contrôlés. La branche est en avance de deux commits sur le distant; aucun push ou nouveau workflow n'a été déclenché.

## 2026-09-04 - File média composée dans le worker générique à 13:42 UTC

- Le heartbeat a poursuivi le développement applicatif : après relecture du PDF maître (71 pages, SHA-256 exact), des pages cœur 3-7, 31-33, 46, 48, 69-71 et des pages 11, 14, 16-18, 22-23, 35-38, 64-65, `masterPrompt.alignment` a été actualisé avant le code. Les pages 48 et 69 ont également été vérifiées en rendu.
- Le worker générique exécute maintenant la file d'import média après ses traitements existants et publie son résumé dans le résultat et les logs structurés du batch. Le mode par défaut est explicitement `not_configured`; le mode `disabled` est également explicite. Dans ces deux états, aucune réservation n'est sélectionnée et aucun fetch, stockage, journal d'exécution ou pièce jointe n'est créé.
- Une composition n'est actionnable que si le provider, le stockage et le coffre sont tous prêts en mode `mock`. La sélection multi-tenant est bornée et, sur PostgreSQL, refuse de s'exécuter hors contexte système. Chaque réservation conserve ensuite son `tenant_id`; les audits utilisent `system_whatsapp_meta` tandis que le créateur réel de l'endpoint reste le responsable référencé par l'exécution.
- Le parcours vertical testé part d'un webhook Meta signé, crée la réservation dans le Conversation Hub, la reprend automatiquement au batch, applique la policy avant déchiffrement et IO, puis crée exactement une pièce jointe canonique mock. Le batch suivant ne refait ni fetch, ni stockage, ni insertion.
- Preuves locales : 43 tests ciblés verts; 2 tests PostgreSQL/RLS ignorés sans `DATABASE_URL`; suite exhaustive 139 fichiers/675 tests verts et 8 fichiers/20 tests PostgreSQL ignorés; ESLint complet, TypeScript, build Next.js production et `git diff --check` verts. Le build a utilisé le réseau uniquement pour les polices Google déjà déclarées. Le lanceur `pnpm agent:continuity-check` reste bloqué par sa tentative de réinstallation, mais le script versionné direct est `ready`, sans erreur ni avertissement.
- GitHub relu sans mutation : PR #11 ouverte, brouillon et `MERGEABLE/UNSTABLE` au head distant `28efa75`; continuité `33826756891` verte; CI `33826756939` inchangée, arrêtée avant tests sur le timeout npm audit. Aucun push, rerun ou déclenchement indirect n'a eu lieu.
- Le code est conservé dans le commit local `9af546df640a555c76da755bd3139ce541a1196f`. Aucun Graph, stockage réel, antivirus, OCR/transcription, secret, message externe, endpoint public, activation, fusion, déploiement ou dépense. `tmp/` reste non suivi et hors index.

## 2026-09-04 - Scan de sécurité média obligatoire avant stockage à 14:46 UTC

- Le heartbeat a poursuivi le développement applicatif avec la première tranche locale non bloquée. Le PDF maître a été revérifié à 71 pages et au SHA-256 exact; les pages cœur 3-7, 31-33, 46, 48 et 69-71 ainsi que 11, 14, 16-18, 22-23, 35-38 et 64-65 ont été relues directement. Les pages 23 et 69 ont été inspectées en rendu et `masterPrompt.alignment` a été mis à jour avant le code.
- Le worker durable impose maintenant une analyse de sécurité injectée après les contrôles de taille, type métier, MIME, signature binaire et SHA-256, et avant le stockage. Une seule composition provider/scanner/storage entièrement `mock` peut agir; `disabled` et `not_configured` ferment le worker générique avant sélection et IO.
- Un verdict propre autorise le stockage avec la même clé d'idempotence. Un verdict dangereux devient un échec de validation définitif sans stockage ni pièce jointe. Une indisponibilité temporaire du scanner est classée, replanifiée puis reprise; le téléchargement peut être rejoué mais le stockage ne s'exécute qu'une fois après verdict propre.
- La migration additive runtime `109_os5_channel_provider_media_security_scan` et son miroir SQL `0103` ajoutent le mode scanner à une base ayant déjà reçu 107/108. Les lignes historiques sont marquées `not_configured`, le mode devient non nul et immuable, et une nouvelle transition vers `succeeded` sans scanner `mock` est refusée en base.
- Les audits exposent seulement provider, modes, état, classification et compteurs. Les tests vérifient l'absence de Media ID, octets, checksum, nom, référence de stockage, signature de malware et détail antivirus.
- Preuves finales ciblées : 2 fichiers/30 tests verts; le fichier PostgreSQL/RLS et ses 2 tests sont ignorés sans `DATABASE_URL`. La régression exhaustive a 139 fichiers/678 tests verts et 8 fichiers/20 tests PostgreSQL ignorés. ESLint complet, TypeScript, build Next.js production, continuity-check direct et `git diff --check` sont verts. Le build a utilisé le réseau uniquement pour les polices Google déjà déclarées.
- Le code est préservé dans le commit local `ca08001929a438f94cf13c889a9f99a0425b67b8`. GitHub est resté inchangé : PR #11 ouverte, brouillon et `MERGEABLE/UNSTABLE` au head distant `28efa75`; continuité `33826756891` verte, CI `33826756939` rouge avant tests uniquement sur timeout npm audit. Aucun push, rerun ou déclenchement indirect n'a été effectué.
- État honnête : livré localement et mock uniquement. Aucun antivirus, Graph, stockage Supabase, OCR/transcription, secret, message externe, endpoint public, activation, fusion, déploiement ou dépense. `tmp/` reste non suivi, préservé et hors index.

## 2026-09-04 - Extraction média cloisonnée comme donnée externe non fiable à 15:50 UTC

- Le développement applicatif a continué après le scanner. Le PDF maître a été revérifié à 71 pages et au SHA-256 exact; les pages cœur 3-7, 31-33, 46, 48 et 69-71 ainsi que les pages média/techniques 10-18, 22-24, 35-38 et 64-65 ont été relues directement. Les pages 23, 38 et 69 ont été inspectées en rendu, puis `masterPrompt.alignment` a été actualisé avant le code.
- Le worker impose désormais la séquence provider mock → validation binaire/hachage → scanner mock propre → extracteur mock → stockage mock. L'extracteur reçoit uniquement les octets et métadonnées bornées; aucun LLM, outil, policy ou fournisseur réel ne lui est exposé. `disabled` et `not_configured` ferment la file générique avant sélection et IO.
- La migration additive runtime `110_os5_media_untrusted_extraction` et son miroir SQL `0104` ajoutent le mode extracteur immuable à l'exécution. L'extraction est conservée sur la pièce jointe sous la frontière fixe `external_untrusted_data`, avec clé d'extracteur, texte borné à 16 000 caractères, SHA-256 et date; les six champs doivent être tous absents ou tous cohérents et deviennent immuables.
- La base refuse toute nouvelle transition d'import vers `succeeded` si l'extracteur n'est pas `mock` ou si la pièce jointe ne porte pas l'enveloppe. Le texte hérite de l'isolation tenant et de la RLS existante de `conversation_message_attachments`; un test PostgreSQL dédié couvre lecture propre et invisibilité cross-tenant, mais reste ignoré localement faute de `DATABASE_URL`.
- Le Conversation Hub distingue les contrats d'entrée et de sortie : un message entrant ne peut pas usurper une extraction, tandis que la lecture canonique expose l'enveloppe persistée. L'écran Conversation affiche « Contenu externe non fiable · extraction mock » et précise que le texte est une donnée incapable de fournir une règle ou une action. Ni hash ni référence technique ne sont affichés.
- Les tests prouvent l'ordre scan → extraction → stockage, le rejeu sans duplication, le retry temporaire de l'extracteur sans stockage prématuré, le refus base sans enveloppe, l'immutabilité et l'absence du texte adversarial dans l'audit. Preuves ciblées : 3 fichiers/38 tests verts et 1 fichier/3 tests PostgreSQL/RLS ignorés. Régression exhaustive : 139 fichiers/681 tests verts et 8 fichiers/21 tests PostgreSQL ignorés.
- ESLint complet, TypeScript, build Next.js production, continuity-check direct et `git diff --check` sont verts. Le build sandbox a d'abord refusé l'accès aux polices Google déjà déclarées, puis le même build autorisé a réussi. Le scénario Playwright vérifie le badge et le texte, mais son exécution attend la CI PostgreSQL partagée.
- Le code est préservé dans le commit local `4cb9f4220cdd6e0d0b17ad3a4239e37d40c512ed`. GitHub reste inchangé : PR #11 ouverte, brouillon et `MERGEABLE/UNSTABLE` au head distant `28efa75`; continuité `33826756891` verte, CI `33826756939` arrêtée avant tests par le timeout npm audit. Aucun push, rerun, Graph, stockage réel, OCR/transcription, secret, message externe, endpoint public, activation, fusion, déploiement ou dépense. `tmp/` reste non suivi et hors index.

## 2026-09-04 - Intégrité de l'extraction vérifiée à la lecture à 16:43 UTC

- Le travail applicatif a continué sur la première tranche locale non terminée. Le PDF maître est resté normatif : SHA-256 exact, 71 pages, pages cœur 3-7, 31-33, 46, 48 et 69-71 et pages candidat 10-18, 22-24, 35-38 et 64-65 relues directement. `masterPrompt.alignment` a été actualisé avant le code.
- Le Conversation Hub recalcule désormais le SHA-256 de chaque texte extrait au moment de la lecture. Une empreinte incohérente, un texte hors borne ou un enregistrement partiel produit uniquement l'état `integrity: failed`; ni texte ni hash ne sont renvoyés au client.
- Le schéma canonique distingue strictement les variantes `verified` et `failed`. L'écran Conversation indique en français « Extraction mock · intégrité vérifiée » ou « Extraction masquée — intégrité non vérifiée. » et n'affiche jamais le contenu altéré.
- Une vue `data-only` séparée prépare le futur usage sûr d'une extraction vérifiée : 8 000 caractères maximum, masquage des URLs, adresses privées, jetons Bearer, affectations de secrets et blocs PEM, avec `instructionsAllowed: false`, `toolAccess: forbidden` et `policyMutation: forbidden`. Elle n'est appelée par aucun planificateur, orchestrateur, LLM ou outil.
- Preuves ciblées : 4 fichiers/40 tests verts, incluant intégrité correcte, hash altéré, enregistrement partiel, non-divulgation, filtrage et lecture de service. La régression exhaustive est verte avec 140 fichiers/685 tests réussis et 8 fichiers/21 tests PostgreSQL ignorés faute de `DATABASE_URL`. ESLint complet, TypeScript et build production sont verts; le build a utilisé le réseau uniquement pour les polices Google déjà déclarées.
- Le scénario Playwright couvre maintenant deux pièces jointes dans le même fil : extraction valide visible et extraction altérée masquée sans fuite. Il est contrôlé par ESLint/TypeScript mais attend PostgreSQL partagé en CI pour être revendiqué exécuté.
- `pnpm agent:continuity-check` a été tenté et a rencontré le défaut connu du wrapper qui veut réinstaller sans TTY; le script versionné direct est `ready`, zéro erreur et zéro avertissement. `git diff --check` est vert.
- Le code est conservé dans les commits locaux `3df4f7c5f642bfca473820a4efe1f42838c3a5c5` et `15b2af7949e937b2f59a5cdd6db9728489fb1478`. GitHub est inchangé : PR #11 ouverte, brouillon et `MERGEABLE/UNSTABLE` au distant `28efa75`; continuité `33826756891` verte, CI `33826756939` rouge avant tests uniquement sur timeout npm audit. Aucun push, rerun, LLM, outil, Graph, stockage réel, OCR, secret, message externe, endpoint public, activation, fusion, déploiement ou dépense. `tmp/` reste non suivi et hors index.

## 2026-09-04 - Accès court et tenant-scoped aux pièces jointes à 17:49 UTC

- Le développement applicatif a continué avec la tranche suivante du Conversation Hub. Le PDF maître a été revérifié à 71 pages et au SHA-256 exact; les pages cœur 3-7, 31-33, 46, 48 et 69-71 ainsi que les pages candidates 10-18, 22-24, 35-38 et 64-65 ont été relues directement. `masterPrompt.alignment` a été actualisé avant le code.
- Un codec injecté émet des tickets AES-256-GCM de 30 à 300 secondes. Le ticket chiffré contient uniquement version, nonce et dates; tenant, utilisateur, pièce jointe et version de clé sont liés comme données associées. Un ticket altéré, expiré ou présenté pour un autre acteur/fichier est refusé avec un message public générique.
- Le service exige le membership puis la policy avant émission et lecture. Il relit la pièce après l'IO et refuse une mutation concurrente. Les octets sont bornés à 25 Mo, leur taille exacte et leur SHA-256 sont recalculés avant restitution, et la clé d'idempotence dérivée du ticket reste stable au rejeu.
- Les frontières de stockage annoncent obligatoirement `disabled`, `not_configured` ou `mock`; les deux premiers états n'appellent aucun fournisseur. Le runtime applicatif est `not_configured` par défaut et l'écran Conversation affiche « Téléchargement non configuré » pour chaque pièce jointe.
- Les erreurs fournisseur sont réduites à un vocabulaire sûr. Les audits succès, refus, expiration, intégrité et mutation ne conservent ni ticket, contenu, nom, checksum ni référence de stockage. Aucun secret ou identifiant fournisseur n'est introduit.
- Preuves ciblées : 5 fichiers conversation/média réussis, 1 fichier PostgreSQL/RLS ignoré, 50 tests réussis et 3 ignorés faute de `DATABASE_URL`. La régression exhaustive termine avec 141 fichiers/693 tests verts et 8 fichiers/21 tests PostgreSQL ignorés. ESLint complet, TypeScript, build Next.js production, continuity-check direct et `git diff --check` sont verts.
- Le premier build a refusé l'environnement de production incomplet; le build final avec valeurs locales factices a nécessité le réseau uniquement pour les polices Google déjà déclarées et a compilé toutes les routes. Aucun secret réel ni base PostgreSQL n'a été utilisé.
- Le scénario Playwright adapté a lancé l'application sur une base temporaire, puis s'est arrêté à `connexion=erreur` avant Conversation parce que le processus test et le serveur ne partagent pas PGlite. Aucune assertion applicative n'est présentée comme verte; la preuve navigateur attend la CI PostgreSQL partagée.
- Le code est conservé dans le commit local `734c8402de5183b840901bd978777d3fb9d16a84`. GitHub reste inchangé : PR #11 ouverte, brouillon et `MERGEABLE/UNSTABLE` au head distant `28efa75`; continuité `33826756891` verte et CI `33826756939` arrêtée avant tests sur timeout npm audit. Aucun push, rerun, Graph, stockage réel, URL durable, message externe, endpoint public, activation, fusion, déploiement ou dépense. `tmp/` reste non suivi et hors index.

## 2026-09-04 - Frontière HTTP/session des pièces jointes à 18:42 UTC

- Le travail applicatif a repris sur l'étape suivante du Conversation Hub. Le PDF maître a été revérifié à 71 pages et au SHA-256 exact; les pages cœur 3-7, 31-33, 46, 48 et 69-71 ainsi que les pages candidates 10-18, 22-24, 35-38 et 64-65 ont été relues directement. `masterPrompt.alignment` a été actualisé avant le code.
- Un handler HTTP injecté et une route Next.js same-origin exposent `POST` pour préparer la capacité courte et `PUT` pour la consommer. Session et tenant sont résolus avant le service; origine, méthode et identifiant sont validés en amont.
- `PUT` exige un JSON UTF-8 strict borné à 4 Kio contenant seulement le ticket. Le ticket ne passe jamais dans l'URL. Les réponses françaises distinguent prêt, refusé, désactivé, non configuré et indisponibilité sans exposer les codes internes.
- Les réponses fixent `private, no-store`, `no-referrer`, `nosniff` et la corrélation. Le binaire fixe type, taille et `Content-Disposition` assaini en ASCII et UTF-8. La route de production utilise le runtime canonique `not_configured` par défaut; aucun fournisseur réel n'est ajouté.
- Dix-sept tests HTTP adversariaux sont verts. Une intégration part d'une base en mémoire, traverse handler, service, ticket chiffré et stockage mock, restitue les octets puis prouve un audit sans ticket, nom, contenu ni référence de stockage. Les ciblés finaux totalisent 3 fichiers/26 tests; la régression ciblée 5 fichiers/61 tests avec 1 fichier/3 tests PostgreSQL ignorés.
- La suite exhaustive source termine avec 142 fichiers/710 tests verts et 8 fichiers/21 tests PostgreSQL ignorés faute de `DATABASE_URL`. ESLint complet, TypeScript, build Next.js production avec la nouvelle route inventoriée, continuity-check et diff check sont verts. L'intégration HTTP, ajoutée ensuite sans modification source, repasse avec les ciblés finaux; PostgreSQL/RLS et Playwright attendent la CI partagée.
- GitHub reste inchangé : PR #11 ouverte, brouillon et `MERGEABLE/UNSTABLE` au head distant `28efa75`; continuité `33826756891` verte, CI `33826756939` rouge avant tests uniquement sur timeout npm audit. Aucun push, rerun, Graph, stockage réel, secret, message externe, endpoint public, activation, fusion, déploiement ou dépense. `tmp/` reste non suivi et hors index.
- Le code est conservé dans les commits locaux `e24356f951a46abaf8cd5f95a37362463f778c0f` et `422317e8452b3cecea9c0514b8306787e65c6bee`. Prochaine tranche non bloquée : confidentialité et visibilité durables du fil, tenant/RLS, transmises à la policy d'accès aux pièces jointes.

## 2026-09-04 - Confidentialité et visibilité héritées du fil à 19:41 UTC

- Le développement applicatif a poursuivi l'ordre conversation-first. Le PDF maître a été confirmé à 71 pages et au SHA-256 exact; les pages cœur 3-7, 31-33, 46, 48 et 69-71 ainsi que 10-12, 17-18, 22-24, 35-38 et 64-65 ont été relues directement. Les pages 11, 23, 48 et 69 ont aussi été inspectées en rendu. `masterPrompt.alignment` a été actualisé avant le code.
- La migration additive runtime `111_os5_conversation_thread_access_classification` et le miroir SQL `0105` ajoutent `confidentiality_level` et `visibility_scope`, non nuls, avec vocabulaires stricts, défauts historiques `internal`/`tenant` et index tenant-leading. La table conserve sa RLS existante.
- Les types, schémas, insertions et lectures du Conversation Hub portent les deux attributs. L'interface affiche des badges français de confidentialité et visibilité sans ajouter de module secondaire.
- Une lecture de pièce jointe joint maintenant attachment, message et thread sur leurs clés tenant. La policy reçoit `threadId`, `confidentialityLevel` et `visibilityScope` à `prepare` et `read`; la revalidation après IO compare aussi ces droits et ferme l'accès s'ils changent pendant la lecture.
- Les tests de migration prouvent parité runtime/SQL, upgrade depuis 110, défauts et contraintes. Le test PostgreSQL restreint couvre select/update/insert cross-tenant mais reste ignoré localement faute de `DATABASE_URL`. Le scénario Playwright vérifie les deux badges et attend la CI PostgreSQL partagée.
- Preuves ciblées : 5 fichiers/21 tests verts et 1 fichier/1 test PostgreSQL ignoré. Régression Conversation/Meta/média : 19 fichiers/151 tests verts et 2 fichiers/4 tests PostgreSQL ignorés. Suite exhaustive : 144 fichiers/715 tests verts et 9 fichiers/22 tests PostgreSQL ignorés. ESLint complet, TypeScript, build Next.js production et diff check sont verts; le build n'a utilisé le réseau que pour les polices Google déjà déclarées.
- GitHub relu sans mutation : PR #11 ouverte, brouillon, `MERGEABLE/UNSTABLE` au head distant `28efa75`; continuité `33826756891` verte et CI `33826756939` inchangée, rouge avant tests sur timeout npm audit. Aucun push, rerun, Graph, stockage réel, secret, message, endpoint public, activation, fusion, déploiement ou dépense; `tmp/` reste non suivi et hors index.
- Le code est conservé dans le commit local `8900ac5836c504c970d32ee88f8f87bdb515a591`. Prochaine tranche non bloquée : autorisations durables de fil pour appliquer les portées `personal`/`team`/`case` dans la policy, avec tenant/RLS et compatibilité `tenant`.

## 2026-09-04 - Autorisations durables des fils à 21:02 UTC

- Le développement applicatif a continué sur le Conversation Hub. Le PDF maître a été confirmé à 71 pages et au SHA-256 exact; les pages cœur 3-7, 31-33, 46, 48 et 69-71 ainsi que 10-14, 17-18, 22-24, 35-38, 47 et 64-65 ont été relues directement. Les pages 11, 22, 48 et 69 ont aussi été inspectées en rendu. `masterPrompt.alignment` a été actualisé avant le code.
- Les migrations additives runtime `112_os5_conversation_thread_access_grants` et `113_os5_conversation_thread_access_grants_rls`, avec miroirs SQL `0106/0107`, ajoutent les droits tenant-owned et le registre d'opérations idempotentes. Les relations composées lient chaque droit au fil et au membership du même tenant; contraintes, unicité et index tenant-leading sont couverts.
- La RLS autorise un utilisateur à lire seulement son droit et propriétaire/administrateur à administrer la configuration. L'historique d'idempotence est immuable pour les rôles ordinaires; le bypass système demeure réservé aux frontières serveur de confiance.
- Le service de configuration applique les cardinalités : `personal` exactement une personne, `team`/`case` au moins une et `tenant` aucune liste. Il valide les memberships, remplace atomiquement les droits, stabilise le rejeu et refuse la collision d'une même clé avec une autre demande. L'audit ne conserve ni clé brute, ni contenu, ni liste d'identifiants utilisateur.
- Les listes et lectures de fils, ainsi que l'ingestion humaine, exigent le droit actif correspondant pour `personal`/`team`/`case`; `tenant` reste accessible aux membres. Le message système conserve un bypass explicite et borné après authentification du canal.
- L'accès aux pièces jointes vérifie le droit avant policy et stockage, puis le relit après l'IO. Une révocation ou mutation concurrente ferme l'accès et produit seulement un code d'audit générique. L'écran Conversation affiche en français « Accès : Membres de l'organisation » ou « Accès : Autorisation explicite requise ».
- Preuves après le dernier durcissement RLS : 6 fichiers ciblés verts et 1 fichier PostgreSQL ignoré, 27 tests verts et 1 ignoré sans `DATABASE_URL`; ESLint ciblé et TypeScript verts. Avant ce dernier changement SQL : régression Conversation/Meta/média 21 fichiers verts et 3 ignorés, 158 tests verts et 5 ignorés; suite exhaustive 146 fichiers verts et 10 ignorés, 722 tests verts et 23 ignorés; lint complet et build Next.js production verts.
- GitHub a été relu sans mutation : PR #11 ouverte, brouillon, `MERGEABLE/UNSTABLE` au head distant `28efa75`; continuité `33826756891` verte et CI `33826756939` rouge avant tests uniquement sur timeout npm audit. Aucun push, rerun, Graph, stockage réel, secret, message, endpoint public, activation, fusion, déploiement ou dépense; `tmp/` reste non suivi et hors index.
- Le code est conservé dans le commit local `8d6e329b363cac96b5cfb793f82abab7e9aee273`. L'audit suivant montre que création, lecture, liste, décision, exécution et reprise des plans contrôlent encore seulement le membership tenant : leur propagation des droits du fil est la prochaine tranche OS-1/OS-4. `goal-watch` reste hors périmètre OS-6.

## 2026-09-04 - Droits des plans et missions hérités du fil à 21:47 UTC

- Le travail applicatif a repris sur la première tranche non terminée de l'ordre directeur : les plans et missions Conversation héritent maintenant des droits actifs de leur fil. Le PDF maître a été confirmé à 71 pages et au SHA-256 exact; les pages cœur 3-7, 31-33, 46, 48 et 69-71 ainsi que les pages métier/techniques 10-12, 15-18, 22, 24, 28, 35-38, 43-44, 64 et 68 ont été relues directement. Les pages 17, 32, 48 et 69 ont aussi été inspectées en rendu. `masterPrompt.alignment` a été actualisé avant le code.
- La création vérifie le droit avant d'appeler le générateur puis avant persistance. Lecture, liste, décision et exécution refusent de façon non énumérable un plan dont le fil `personal`, `team` ou `case` est devenu inaccessible; les fils `tenant` restent compatibles.
- La reprise manuelle revalide le droit dans la transaction qui modifie réellement la mission. Une révocation injectée entre les deux transactions laisse l'état et le compteur intacts, sans événement de reprise ni audit. La liste générique des missions masque également les plans inaccessibles et ses commandes annuler/approuver/refuser/relancer appliquent la même garde.
- La finalisation système asynchrone d'une mission déjà autorisée reste bornée, durable et idempotente; elle ne crée aucun droit utilisateur et ne rend aucun résultat accessible hors du fil protégé.
- Preuves : 3 fichiers ciblés et 16 tests verts; régression orchestrateur/workflows/Conversation avec 14 fichiers verts, 1 PostgreSQL ignoré, 53 tests verts et 1 ignoré; suite exhaustive avec 146 fichiers verts, 10 ignorés, 724 tests verts et 23 ignorés sans `DATABASE_URL`. ESLint complet, TypeScript, build Next.js production, continuity-check et diff check sont verts.
- GitHub a été relu sans mutation : PR #11 ouverte, brouillon, `MERGEABLE/UNSTABLE` au head distant `28efa75`; continuité `33826756891` verte et CI `33826756939` rouge avant tests uniquement sur timeout npm audit. Aucun push, rerun, LLM fournisseur, Graph, stockage réel, secret, message, activation, fusion, déploiement ou dépense; `tmp/` reste non suivi et hors index.
- Le code est conservé dans le commit local `754316a5466ca16b730ef0e521852dcce3459b7e`. La prochaine tranche locale est le filtrage des événements actifs, de la file et des incidents/dead letters rattachés à un plan inaccessible, avec leurs contrôles génériques; `goal-watch`/OS-6 reste hors périmètre.

## 2026-09-04 - Événements et incidents héritent des droits du fil à 22:48 UTC

- Le développement applicatif a continué sur la tranche OS-4 suivante. Le dépôt Developer est resté lisible et inscriptible, `tmp/` a été préservé, le PDF maître confirmé à 71 pages et au SHA-256 exact, puis les pages cœur 3-7, 31-33, 46, 48 et 69-71 et les pages 10-12, 15-18, 22, 24, 28, 35-38, 43-44, 64 et 68 ont été relues directement. Les pages 17, 32, 48 et 69 ont été inspectées en rendu; `masterPrompt.alignment` a été actualisé avant le code.
- La file et les dead letters chargent le payload uniquement dans le repository pour résoudre le plan par `planId` ou la mission par `runId`; aucune réponse publique ne renvoie ce payload. Les événements d'un fil révoqué sont masqués, tandis que les événements génériques et les fils `tenant` restent compatibles.
- L'annulation d'un événement actif et la relance d'un incident revalident le droit dans leur transaction de mutation. Le refus est non énumérable et laisse statut, nombre de tentatives et audit inchangés. La lecture paginée par lots de 50 empêche un grand nombre d'événements masqués d'évincer une ligne générique accessible.
- Les agrégats tenant-wide conservent seulement statut, compte et dates opérationnelles, sans identifiant, corrélation ni contenu. Le test vertical place treize reprises restreintes avant une ligne générique et vérifie la non-fuite, la non-mutation et l'absence d'audit de contrôle.
- Preuves : ciblés 3 fichiers/27 tests verts; régression Conversation/workflows/vues 21 fichiers/96 tests verts avec 2 fichiers/2 tests PostgreSQL ignorés; suite exhaustive 146 fichiers/724 tests verts et 10 fichiers/23 tests ignorés sans `DATABASE_URL`. ESLint complet, TypeScript, build Next.js production et diff check sont verts. Le premier build a correctement refusé une configuration de cookie incohérente, le second a échoué uniquement faute de réseau Google Fonts, puis le build autorisé a réussi.
- Le changelog officiel Supabase ne signale aucune rupture pertinente pour cette tranche de service. GitHub a été relu sans mutation : PR #11 ouverte, brouillon, `MERGEABLE/UNSTABLE` au head distant `28efa75`; continuité `33826756891` verte et CI `33826756939` rouge avant tests uniquement sur timeout npm audit. Aucun push, rerun, Graph, stockage réel, secret, message, activation, fusion, déploiement ou dépense; `tmp/` reste hors index.
- Le code est conservé dans le commit local `517a8a73acf9f4f07afe254dd84c8489e2f6e37a`. L'audit suivant identifie une frontière RLS intra-tenant encore seulement tenant-scoped sur les objets Conversation dérivés; une migration additive runtime/SQL et des tests restricted-role sont la prochaine tranche, sans OS-6.

## 2026-09-04 - RLS intra-tenant héritée du fil à 23:27 UTC

- Le travail applicatif a continué sur la première tranche non terminée de l'ordre directeur. Le dépôt exact `/Users/TRADIKOM/Developer/TRADIKOM-ONE` est resté lisible et inscriptible et `tmp/` a été préservé. Le PDF maître a été confirmé à 71 pages et au SHA-256 exact; les pages cœur 3-7, 31-33, 46, 48 et 69-71 ainsi que les pages RLS 10-12, 16-18, 22, 28, 35-38 et 43-44 ont été relues directement. `masterPrompt.alignment` a été actualisé avant le code.
- La migration runtime `114_os5_conversation_derived_access_rls` et son miroir `0108_os5_conversation_derived_access_rls.sql` installent six helpers `SECURITY INVOKER`, sans `SECURITY DEFINER`. Ils exigent tenant, `app.actor_id`, membership et droit actif du fil pour les portées `personal`, `team` et `case`; les fils `tenant` restent accessibles aux membres.
- Onze tables disposent maintenant de politiques séparées `SELECT`, `INSERT`, `UPDATE` et `DELETE` : fils, participants, messages, pièces jointes, hops, plans, étapes, missions, étapes de mission, validations et événements. Les mises à jour contrôlent l'ancienne et la nouvelle ligne. Les workflows `conversation_plan:*` héritent du plan et du fil; les workflows, validations et événements génériques restent tenant-scoped.
- Les événements résolvent prudemment `planId` ou `runId`; un JSON invalide devient un objet vide et `conversation.plan.execute` sans plan résolu échoue fermé. Un rôle restreint ne peut pas forger `app.system_access=true`, car le bypass exige aussi l'appartenance au rôle propriétaire. Seule la configuration des droits par propriétaire/administrateur emploie le contexte serveur tenant+acteur+system explicite, après le contrôle de rôle dans la même transaction.
- Les anciens tests PostgreSQL ont été alignés sur le contexte acteur et sur les quatre politiques par opération. Le nouveau scénario restricted-role couvre l'acteur autorisé, le membre exclu, un autre tenant, lectures et mutations, objets génériques et drapeau système forgé.
- Preuves locales : toutes les migrations RLS se chargent sous PGlite; ciblés 3 fichiers/14 tests verts et 1 test PostgreSQL ignoré; régression Conversation/workflows 21 fichiers/105 tests verts et 3 ignorés; suite exhaustive 147 fichiers/727 tests verts et 11 fichiers/24 tests ignorés. ESLint complet, TypeScript, build Next.js production et `git diff --check` sont verts. Le build final n'a utilisé le réseau que pour les polices Google déjà déclarées.
- `pnpm db:verify` et le test restricted-role PostgreSQL ne peuvent pas s'exécuter localement sans `DATABASE_URL`; aucune base ni aucun secret n'a été demandé. Le changelog et la documentation RLS Supabase ont été relus sans rupture pertinente identifiée. La CI doit confirmer migrations, `db:verify`, RLS, suite exhaustive et Playwright.
- GitHub a été relu sans mutation : PR #11 ouverte, brouillon et `MERGEABLE/UNSTABLE` au head distant `28efa750935b2766de3410b8d9c0d5e3c4e2dbe8`; continuité `33826756891` verte et CI `33826756939` rouge avant tests uniquement sur timeout npm audit. Aucun push, rerun, Graph, stockage réel, secret, message, activation, fusion, déploiement ou dépense.
- Le code est conservé dans le commit local `8f156fb03b1b76406dea1c864b6e9f6435f81115`. Reprise exacte : attendre « publie et lance la CI », revalider le head distant, pousser seulement en fast-forward sûr, puis exiger audit, migrations, `db:verify`, restricted-role/RLS, suite exhaustive, build et Playwright avant toute autre tranche.

## 2026-09-05 - Publication et preuve CI de la RLS Conversation à 02:06 UTC

- L'utilisateur a autorisé la publication et la CI. Le dépôt exact `/Users/TRADIKOM/Developer/TRADIKOM-ONE` a été vérifié lisible et inscriptible; `tmp/` est resté non suivi et hors index. Avant chaque push, le head distant a été récupéré, comparé et confirmé ancêtre du head local; aucun reset, clean, stash, changement de branche ou écrasement.
- Le PDF maître a été résolu dans Downloads, vérifié à 71 pages et au SHA-256 `bb838fb02c23247b1bcda8981539eebe73264a5334bfaf565aafa5bc26c50fe5`. Les pages cœur 3-7, 31-33, 46, 48 et 69-71 et les pages RLS/métier 10-12, 16-18, 22, 28, 35-38 et 43-44 ont été relues directement; les pages 48 et 69 ont aussi été contrôlées en rendu. Le continuity-check local était `ready`, zéro erreur et zéro avertissement.
- Les vingt-sept commits applicatifs/documentaires ont d'abord été publiés jusqu'à `041459a`. La continuité `33934389549` a réussi. La CI `33934389487` a franchi l'audit mais révélé un écart réel du vérificateur : il exigeait une policy `ALL` unique et rejetait les tables correctement protégées par quatre policies `SELECT`/`INSERT`/`UPDATE`/`DELETE`.
- Le commit `1ca6e83aa56fe0231f5df1300ade4e72f2f863df` ajoute `scripts/tenant-rls-coverage.ts`, utilisé par le vérificateur et le test global PostgreSQL. La fonction et la requête acceptent `ALL` ou les quatre opérations complètes, mais refusent une opération manquante et RLS désactivée. Un test PGlite crée et interroge les quatre variantes. Tests ciblés, ESLint et TypeScript étaient verts.
- La CI `33934691922` a ensuite dépassé migrations, `db:verify`, sauvegarde/restauration, lint et typecheck, puis révélé uniquement un paramètre SQL inutilisé et des identifiants de fixtures fixes. Le commit `8c9ddbc91653d522800104139dcacdafe497126f` rend les fixtures Conversation/média uniques et corrige ce premier paramétrage.
- La CI `33935855879` a de nouveau passé toute la chaîne avant tests et révélé les derniers paramètres SQL clairsemés ainsi que l'absence de contexte acteur dans l'extraction média désormais protégée. Le commit `25cafcdaa9031cf7835068cc2487d921c0ff6d51` rend tous les placeholders contigus, crée le membership attendu et transmet `app.actor_id`. Un audit AST des deux fichiers a trouvé zéro paramètre manquant ou surnuméraire; les ciblés, ESLint et TypeScript sont verts.
- Preuve finale autoritative : continuité `33936955672` verte; CI `33936955678` verte en 22 min 49 s avec audit, migrations et `db:verify`, sauvegarde/restauration, lint, TypeScript, 159 fichiers/757 tests sur PostgreSQL 17, build Next.js production et 20/20 Playwright. La PR #11 est ouverte, brouillon et fusionnable au head distant `25cafcd`.
- État honnête : la RLS Conversation et les tranches média/droits qu'elle protège sont livrées, publiées et prouvées CI. Les fournisseurs réels restent non configurés; aucun token, Graph, stockage Supabase réel, antivirus, OCR/transcription, message externe, endpoint public, fusion, déploiement ou dépense n'a été déclenché.
- Reprise exacte : l'utilisateur saisit lui-même le code SMS dans Meta for Developers et indique seulement que l'étape est terminée. Ensuite, inventorier l'application, le WABA et le Phone Number ID en lecture seule, puis demander une confirmation immédiatement avant tout token persistant. Ne pas démarrer `goal-watch`/OS-6, CRM, Kanban ou dashboard secondaire.

## 2026-09-05 - Checkpoint d'activation Meta visible dans Conversation à 03:32 UTC

- Le développement applicatif a repris sur la première tranche OS-5 non bloquée. Le dépôt exact `/Users/TRADIKOM/Developer/TRADIKOM-ONE` a été confirmé courant, lisible et inscriptible; local et distant étaient synchronisés sur `ed4aeea` avant le travail et `tmp/` est resté non suivi, intact et hors index.
- Le PDF maître a été revérifié à 71 pages et au SHA-256 exact. Les pages cœur 3-7, 31-33, 46, 48 et 69-71 et les pages OS-5 10-18, 22-24, 26-33, 35-38, 43-44 et 64-68 ont été relues directement; les pages 48 et 69 ont aussi été contrôlées en rendu. `masterPrompt.alignment` a été actualisé avant le code.
- Le registre fournisseur savait déjà maintenir `disabled`, `not_configured` et `awaiting_human_auth` sans transport. Le manque applicatif était l'absence de cette vérité dans l'écran principal Conversation.
- Un modèle de présentation Meta couvre maintenant les cinq états du contrat : désactivé, configuration requise, validation humaine requise, simulation et prêt techniquement. Il retourne seulement un libellé français, un résumé, une prochaine action et la nature de l'effet externe; il ne propage ni `missingEnvironment`, ni nom de variable, ni valeur sensible.
- Conversation rend la carte après authentification. Aucun bouton d'activation ou d'envoi n'est présent. Les trois états préparatoires annoncent explicitement qu'aucun message externe ne peut partir; le mock reste simulé; l'état prêt rappelle l'endpoint tenant et l'autorisation séparée nécessaires.
- Les tests unitaires couvrent les cinq états, la validation du bon provider, l'absence de réseau et la non-fuite. Le scénario Playwright existant contrôle désormais la carte sur desktop et mobile, l'état désactivé par défaut, l'effet externe bloqué et l'absence de boutons d'action externe.
- Preuves locales : 3 fichiers/33 tests ciblés verts; suite exhaustive 160 fichiers/740 tests verts et 11 fichiers/24 tests PostgreSQL ignorés faute de `DATABASE_URL`; ESLint complet, TypeScript, build Next.js production, continuity-check et `git diff --check` verts. L'audit au seuil high passe avec zéro high/critical et deux avis transitifs `qs` modérés documentés.
- Le code et l'alignement préalable sont conservés au commit local `c038df459e8e883bb5c77c6932b7a577a8f56f85`. Le Playwright modifié et les contrôles PostgreSQL attendent la CI partagée avant de déclarer la tranche publiée et prouvée.
- La boîte d'inscription Meta déjà ouverte n'a fourni aucun signal fiable confirmant la saisie du SMS. Aucun état humain n'a été inventé; aucun secret, token, Graph, message, endpoint public, stockage réel, fusion, déploiement ou dépense n'a été déclenché.
- Le distant a été récupéré de nouveau avant publication : `ed4aeea` était toujours l'ancêtre exact du head local. Le push `ed4aeea..d899d95` est resté strictement fast-forward et n'a inclus que les deux commits contrôlés; `tmp/` est demeuré hors index.
- La continuité `33942266310` est verte. La CI `33942266316` est entièrement verte en 22 min 56 s : audit, migrations et `db:verify`, sauvegarde/restauration, lint, TypeScript, 160 fichiers/764 tests sur PostgreSQL 17, build Next.js production et 20/20 Playwright.
- Le parcours Conversation desktop/mobile exécute réellement le nouveau contrôle : carte visible, état désactivé par défaut, effet externe bloqué, texte d'absence de message externe et aucun bouton d'activation/envoi.
- État final de la tranche : livré au commit `c038df459e8e883bb5c77c6932b7a577a8f56f85`, publié avec le handoff `d899d95e7d3c590b5fe603d8269fdf1fe9203807` et prouvé CI. La PR #11 reste ouverte, brouillon et fusionnable; aucune fusion ou activation externe.

## 2026-09-05 - Préparation Meta propre au tenant visible dans Conversation à 04:49 UTC

- Le développement applicatif a continué sans attendre le checkpoint humain Meta. Le dépôt exact `/Users/TRADIKOM/Developer/TRADIKOM-ONE` a été confirmé lisible et inscriptible, synchronisé sur `b72a974`; `tmp/` est resté non suivi, intact et hors index.
- Le PDF maître a été résolu dans Downloads, confirmé à 71 pages et au SHA-256 exact. Les pages cœur 3-7, 31-33, 46, 48 et 69-71 et les pages OS-5 10-18, 22, 24, 29-30 et 64-68 ont été relues directement; les pages 48 et 69 ont aussi été contrôlées en rendu. `masterPrompt.alignment` a été actualisé avant le code.
- Le changelog Supabase et la documentation RLS actuels ont été relus. Aucun changement pertinent ne remet en cause la lecture directe PostgreSQL; aucune table ni exposition Data API n'a été ajoutée. La requête s'appuie sur les index tenant/provider et secret actif existants.
- Une lecture agrégée sous transaction tenant+acteur vérifie d'abord le membership puis ne retourne que trois booléens : présence d'une configuration Meta, présence d'une configuration active et présence d'accès chiffrés actifs. Les identifiants d'application, compte, numéro, endpoint, versions, ciphertexts et secrets ne quittent jamais le repository.
- Le service classe quatre états : canal non relié, canal suspendu, accès sécurisé manquant et organisation prête. Une révocation fait repasser l'organisation à l'état incomplet. Un acteur extérieur au tenant reçoit un refus non énumérable.
- Conversation charge cette lecture en parallèle des fils et affiche séparément l'état du serveur et celui de l'organisation, en français. Même avec un manifeste serveur `mock` ou `ready`, l'effet externe reste bloqué si l'organisation n'est pas prête. Aucun bouton d'activation/envoi, Graph, message, endpoint public ou autre effet externe n'a été ajouté.
- Preuves locales : 4 fichiers/21 tests ciblés verts, puis 4 fichiers/24 tests verts et 1 test PostgreSQL ignoré sans `DATABASE_URL`; suite exhaustive 161 fichiers/749 tests verts et 11 fichiers/24 tests PostgreSQL ignorés. ESLint complet, TypeScript, build Next.js production, audit high, continuity-check et `git diff --check` sont verts. Deux avis transitifs `qs` modérés subsistent, zéro high/critical.
- Le scénario PostgreSQL restricted-role couvre désormais la lecture prête du bon tenant et le refus cross-tenant. Playwright couvre une organisation prête sur desktop et un canal non relié sur mobile, tous deux avec serveur désactivé et effet externe bloqué. Ces preuves attendent la CI partagée.
- Le commit `461156ae3ad7f44920fe64dc182780957cd2aaa2` a été poussé strictement en fast-forward après fetch et contrôle d'ancêtre du head distant `b72a974`. Aucun reset, clean, stash, changement de branche ou écrasement.
- État honnête : livré et publié, CI en attente. Aucun compte finalisé, token réel, Graph, message, stockage externe, endpoint public, fusion, déploiement ou dépense. La validation SMS reste une intervention humaine distincte et non déduite.
- La CI `33945539636` a validé l'audit, toutes les migrations, `db:verify`, sauvegarde/restauration, lint, TypeScript, 161 fichiers/773 tests sur PostgreSQL 17 et le build production. Dix-neuf Playwright sur vingt sont passés; le dernier a échoué uniquement sur le mode strict du sélecteur « Désactivé », désormais présent à la fois dans le badge et le détail serveur.
- Le correctif `82586e89045538fddf61fdb56fc563eabd6ed688` limite l'assertion à la zone « État du serveur ». ESLint ciblé, TypeScript et diff check sont verts. Le commit a été publié en fast-forward après confirmation du head distant `461156a`; la CI `33946531702` et la continuité `33946531695` ont été déclenchées sans autre modification applicative.
- La continuité `33946531695` est verte. La CI autoritative `33946531702` est entièrement verte en 18 min 1 s : audit, migrations, `db:verify`, sauvegarde/restauration, lint, TypeScript, 161 fichiers/773 tests sur PostgreSQL 17, build Next.js production et 20/20 Playwright. Les deux nouveaux états tenant visibles dans Conversation sont donc prouvés sur desktop et mobile, et le scénario restricted-role confirme l'isolation entre organisations.
- La PR #11 reste ouverte, brouillon, fusionnable et `CLEAN` au head `82586e89045538fddf61fdb56fc563eabd6ed688`. Aucun Graph, secret réel, token persistant, message externe, endpoint public, fusion, déploiement ou dépense n'a été déclenché. La reprise exacte retourne à la validation SMS effectuée directement par l'utilisateur dans Meta, puis à l'inventaire officiel en lecture seule.

## 2026-09-08 - Autorisation d'essai WhatsApp Meta durable avant transport à 13:14 UTC

- Le travail applicatif a repris dans la copie stable `/Users/TRADIKOM/Developer/TRADIKOM-ONE`, confirmée courante, lisible et inscriptible. Tous les changements existants ont été préservés; `tmp/` reste non suivi, intact et hors index. Local et distant ont été récupérés et confirmés identiques sur `19c5c10cbabd82c14ed08e401f627e932b28172a`; PR #11 ouverte, brouillon, fusionnable et `CLEAN`.
- Le PDF maître est confirmé à 71 pages et au SHA-256 `bb838fb02c23247b1bcda8981539eebe73264a5334bfaf565aafa5bc26c50fe5`. Les pages cœur 3-7, 31-33, 46, 48 et 69-71 et les pages OS-5 10-18, 22-24, 29-30 et 64-68 ont été relues directement; `pnpm agent:continuity-check` retourne `ready`, zéro erreur et zéro avertissement.
- La migration runtime `115_os5_whatsapp_meta_trial_authorization` et son miroir `0109_os5_whatsapp_meta_trial_authorization.sql` étendent strictement les autorisations/consommations à `whatsapp_meta` et `meta_whatsapp_trial`, imposent `max_messages = 1`, ajoutent une FK provider/tenant/endpoint et lient immuablement chaque livraison Meta à son autorisation. Les anciennes livraisons Meta non terminales sans lien durable deviennent `denied` et leur message canonique `failed`; les résultats historiques terminaux restent inchangés.
- Le service émet une autorisation administrative idempotente, expirante et révocable sans donnée fournisseur; le budget verrouille livraison puis autorisation, rejoue la même consommation et refuse atomiquement une seconde livraison. Le worker tenant-wide n'est plus couplé à l'acteur créateur et reprend après une interruption avant ou après consommation à partir du lien persistant.
- Le transport HTTP est marqué structurellement `http`; un manifeste mock n'accepte qu'un transport `mock`. Un état mock ne peut donc jamais atteindre les résolveurs, credentials, destination ou `fetch`. En ready, absence, expiration, révocation, mismatch, cross-tenant ou épuisement échouent avant tout transport.
- La readiness tenant agrège seulement `not_checked`, `required`, `valid` ou `exhausted`, sans ID, date, secret ni ciphertext. Conversation affiche un troisième pavé français « Autorisation d'essai »; aucun bouton Activer/Envoyer ni appel externe n'est ajouté. Playwright prépare un état requis, émet une autorisation puis consomme sa preuve en base pour rendre valide puis épuisé, sans adapter Meta ni réseau.
- Le runner PostgreSQL enveloppe l'historique et tous les DDL dans une transaction sous verrou advisory, afin que deux démarrages concurrents ne doublent pas le journal. Les appels de migration redondants par requête ont été supprimés. La migration 0097 est également atomique et valide la FK composite avant suppression de l'ancienne. Le vérificateur RLS refuse désormais une policy publique permissive non bornée, y compris à côté d'une policy sûre.
- Preuves locales fiables : `pnpm typecheck`, `pnpm lint`, `git diff --check` et l'audit production au seuil high passent; zéro high/critical et deux avis transitifs modérés. Les migrations ciblées passent 7 + 3 + 4 tests; checkpoint/RLS/transport 60 tests; worker/readiness/transport 57 tests. Les scénarios Meta ayant expiré pendant une suspension repassent isolément, y compris unique consommation ready, idempotence, rôles, cross-tenant, révocation et zéro I/O.
- La suite exhaustive locale est non concluante : l'environnement suspend la tâche et Vitest comptabilise alors 15 à 35 minutes sur des tests historiques différents pendant quelques secondes murales. Chaque échec Meta concerné repasse isolément; aucun échec d'assertion stable n'est reproduit. Le build production refuse correctement l'absence de `DATABASE_URL`; `db:verify`, restricted-role PostgreSQL, build et Playwright doivent être validés par la CI partagée après publication.
- État honnête : livré localement, CI en attente; réel connecté = aucun; sandbox = aucune; mock = doubles sans réseau; bloqué humain = SMS Meta, inventaire officiel et confirmation avant token; hors périmètre = Graph, message externe, endpoint public, fusion, déploiement, dépense, CRM, Kanban, dashboard secondaire et OS-6.

## 2026-09-09 - Fermeture au plus une fois, garde RLS et correctifs Next.js à 21:31 UTC

- Le travail a continué dans `/Users/TRADIKOM/Developer/TRADIKOM-ONE`; aucun fichier de la copie iCloud n'a été utilisé. `tmp/` est resté intact et hors index. Le head local et le head distant ont été confirmés identiques sur `19c5c10cbabd82c14ed08e401f627e932b28172a` avant le lot.
- Le PDF maître est toujours exact : SHA-256 `bb838fb02c23247b1bcda8981539eebe73264a5334bfaf565aafa5bc26c50fe5`, 71 pages. Les pages cœur 3-7, 31-33, 46, 48 et 69-71 et les pages OS-5 10-18, 22-24, 29-30 et 64-68 ont été relues directement.
- Une revue adversariale a trouvé qu'une consommation Meta rejouée pouvait encore traverser le transport une seconde fois après un crash entre acceptation externe et finalisation DB. Le service reconnaît maintenant `replayed` avant tout credentials, destination ou adaptateur et terminalise la livraison/message en `failed/permanent/channel_provider_activation_transport_outcome_uncertain`, sans seconde émission.
- Le scénario exact est testé : livraison claimée, budget consommé, transport accepté une fois, crash simulé sans finalisation, expiration du lease, reprise worker puis fermeture permanente; le compteur transport et la consommation restent tous deux à un. Une autorisation expirée ou révoquée après consommation suit la même fermeture incertaine; avant consommation, elle reste refusée sans I/O.
- Le vérificateur RLS contrôle désormais chaque policy permissive, même affectée à un rôle spécifique. Les expressions tautologiques et les fonctions `app_actor_can_access_*` inventées sont rejetées; l'allowlist contient uniquement les cinq helpers d'accès acteur réellement définis par la migration Conversation. L'historique PGlite complet avec RLS activée retourne zéro écart de couverture.
- L'audit courant a révélé deux avis Next.js critiques et un avis `sharp` high sur les versions précédentes. `next` et `eslint-config-next` sont passés à 16.3.4 et l'override `sharp` à `>=0.35.4`; l'installation résout bien Next.js 16.3.4 et sharp 0.35.4. L'audit au seuil high repasse avec seulement trois avis modérés.
- Preuves locales après tous les changements : sous-matrice 3 fichiers/33 tests verts; matrice Meta/RLS/migrations 12 fichiers/109 tests verts; suite exhaustive 152 fichiers/784 tests verts et 11 fichiers/25 tests PostgreSQL ignorés faute de base locale; ESLint, TypeScript et `git diff --check` verts. Sans `DATABASE_URL`, les preuves PostgreSQL réelles, le build production et Playwright restent réservés à la CI.
- Le commit applicatif `dde51f5b338e099d5d9f5ffab2f76b0f5e9e1a8f` a été publié strictement en fast-forward après confirmation que le head distant `19c5c10` était son parent exact. `tmp/` est resté non suivi et hors index.
- La continuité `34409452521` est verte. La CI `34409452423` est entièrement verte en 23 min 30 s : audit, migrations et `db:verify`, sauvegarde/restauration, lint, TypeScript, 163 fichiers/809 tests PostgreSQL inclus, build production Next.js 16.3.4 et 20/20 Playwright. Les scénarios au plus une fois, readiness requise/valide/épuisée et garde RLS sont donc prouvés dans l'environnement autoritatif.
- Aucun appel Graph, secret, token, SMS, message externe, endpoint public, fusion, déploiement ou dépense n'a été effectué. La prochaine action revient au checkpoint humain SMS Meta, puis à l'inventaire officiel en lecture seule et à une confirmation distincte avant tout token ou effet Graph.

## 2026-09-10 - Cohérence endpoint-secret et contrôle d'essai Meta depuis Conversation à 00:35 UTC

- Le développement applicatif a continué dans la copie stable `/Users/TRADIKOM/Developer/TRADIKOM-ONE`; la copie iCloud n'a pas été utilisée et `tmp/` est resté intact, non suivi et hors index. Local et distant étaient synchronisés sur `ed5a571d258542df8980095fb5326376c26a8072` avant cette tranche.
- Le PDF maître a été résolu dans Downloads, confirmé à 71 pages et au SHA-256 exact. Les pages cœur 3-7, 31-33, 46, 48 et 69-71 ainsi que les pages OS-5 10-18, 22-24, 29-30 et 64-68 ont été relues directement; le continuity-check retourne `ready`, zéro erreur et zéro avertissement. `masterPrompt.alignment` a été actualisé avant le code.
- Un module unique calcule l'empreinte HMAC de destination lors de l'enregistrement d'un endpoint et de la rotation d'un secret. Le service Meta verrouille l'endpoint, relit WABA et empreinte, puis compare en temps constant avec la même clé. Un Phone Number ID différent, une clé HMAC erronée ou une identité incompatible est refusé avant mutation et audit; le couple exact et son rejeu restent stables.
- Deux commandes tenant-aware permettent aux seuls propriétaire et administrateur d'autoriser ou révoquer depuis Conversation un essai plafonné à un message. Les actions utilisent uniquement la session, exigent une confirmation explicite, ne reçoivent aucun endpoint du navigateur et traduisent les erreurs en réponses françaises sûres.
- L'émission résout exactement un endpoint Meta actif avec secret courant, rejoue d'abord la même clé d'idempotence, refuse toute seconde clé tant qu'une preuve est valide et interdit définitivement une nouvelle autorisation après consommation de l'endpoint. Zéro ou plusieurs endpoints configurés produisent un refus fermé.
- La révocation verrouille tous les endpoints Meta du tenant puis annule toutes leurs preuves valides et non consommées. Elle reste disponible en état ambigu et reste correcte si le secret a été retiré ou l'endpoint désactivé. Le budget prend ses verrous dans l'ordre endpoint → livraison → autorisation.
- Les lectures concurrentes ont été durcies contre les snapshots périmés : endpoints verrouillés dans une première requête, secrets actifs relus ensuite. Deux tests PostgreSQL tiennent une consommation dans une transaction distincte, prouvent le verrou via `pg_blocking_pids`, puis vérifient qu'une émission ne recrée rien et qu'une révocation ne marque pas comme révoquée une preuve déjà consommée.
- La carte Conversation présente les états requis, valide, épuisé et configuration ambiguë depuis la base. Les bannières après redirection décrivent l'état durable plutôt qu'un succès fourni par l'URL; elles utilisent une région live et ne montrent ni ID, empreinte, date, secret, bouton Envoyer/Activer ou promesse d'effet externe.
- Playwright utilise le vrai parcours UI autoriser → révoquer → autoriser. Seule une consommation synthétique finale est préparée en base pour prouver l'état épuisé; un espion du navigateur exige qu'aucune requête Graph ne parte.
- Les revues indépendantes sécurité, PostgreSQL et interface ont relevé puis fait corriger le rejeu après consommation, l'ordre de verrouillage, la relecture du secret après attente, la révocation multi-endpoint, l'état ambigu, la persistance d'épuisement, les erreurs publiques et les bannières forgeables. La revue finale ne conserve aucun défaut P0-P2.
- Preuves locales finales : ciblés 6 fichiers/62 tests verts; suite exhaustive 152 fichiers/809 tests verts et 11 fichiers/27 tests PostgreSQL ignorés faute de `DATABASE_URL`; ESLint, TypeScript, audit production au seuil high et `git diff --check` verts. L'audit conserve trois avis modérés, zéro high/critical. Le build production refuse correctement `APP_URL`, `CONNECTOR_ENCRYPTION_KEY` et `DATABASE_URL` absents; Playwright local s'arrête sur l'isolation multiprocessus PGlite. Aucune de ces deux exécutions n'est présentée comme verte; la CI PostgreSQL est autoritative.
- État avant publication : livré et prouvé localement, CI en attente; réel connecté = aucun; sandbox = aucune; mock = fixtures/doubles seulement; bloqué humain = SMS Meta, inventaire officiel et token; hors périmètre = Graph, message externe, endpoint public, fusion, déploiement, dépense, CRM, Kanban, dashboard secondaire et OS-6.
- Le commit `cc0bb2b7ff906bba145f26817f43a3ed55b4dd88` a été publié strictement en fast-forward après fetch et confirmation que `ed5a571d258542df8980095fb5326376c26a8072` était son parent distant exact. `tmp/` est resté hors index; aucun reset, clean, stash, changement de branche ou écrasement.
- La continuité `34422242320` est verte. La CI `34422242316` est entièrement verte en 22 min 42 s : audit, migrations et `db:verify`, sauvegarde/restauration, lint, TypeScript, 163 fichiers/836 tests sur PostgreSQL 17, build Next.js 16.3.4 et 20/20 Playwright. Les deux courses transactionnelles, le parcours UI autoriser/révoquer/autoriser et l'absence de requête Graph sont donc prouvés dans l'environnement autoritatif.
- État final de la tranche : livré, publié et prouvé CI; réel connecté = aucun; sandbox = aucune; mock = fixtures/doubles seulement. La PR #11 reste ouverte, brouillon, fusionnable et `CLEAN`. Aucun Graph, secret, token, message externe, endpoint public, fusion, déploiement ou dépense. Prochaine action exacte : saisie humaine du SMS Meta, puis inventaire officiel en lecture seule et confirmation distincte avant tout token persistant ou effet Graph.

## 2026-09-10 - Contexte externe vérifié raccordé aux plans Conversation à 02:50 UTC

- Le travail applicatif a continué dans la copie stable `/Users/TRADIKOM/Developer/TRADIKOM-ONE`; aucun fichier de la copie iCloud n'a été utilisé. Le commit fonctionnel `bf54862aee344accd23af9c7a5fba3d856484cd1` a été publié strictement en fast-forward et `tmp/` reste non suivi, intact et hors index.
- Le PDF maître a été résolu dans Downloads, confirmé à 71 pages et au SHA-256 exact. Les pages cœur 3-7, 31-33, 46, 48 et 69-71 ainsi que les pages candidates OS-5 10-18, 22-24, 26-30, 35-38, 43-44 et 64-68 ont été relues directement; `masterPrompt.alignment` a été actualisé avant le code.
- Les plans Conversation peuvent maintenant recevoir le contenu borné et filtré d'extractions `external_untrusted_data` à intégrité vérifiée, liées au même tenant, fil et message source encore accessible. Les marqueurs restent immuables : aucune instruction, aucun outil et aucune mutation de policy; dix sources et 16 000 caractères au total au maximum.
- Le contenu externe reste éphémère. Le plan durable ne conserve qu'une provenance sûre, sans contenu, extracteur, empreinte ou identifiant visible. L'absence de source conserve la sérialisation et le fingerprint historiques, tandis que le lecteur matérialise une liste vide pour les anciens plans.
- Avant persistance, le service relit les droits et le contexte. Il verrouille le message source `FOR UPDATE`, les pièces jointes existantes `FOR SHARE`, puis compare le fingerprint; suppression, insertion, altération ou révocation concurrente échoue avant plan, validation, message et audit.
- L'interface affiche en français la provenance vérifiée et rappelle qu'elle est traitée comme donnée non fiable, sans instruction ni accès outil. Les erreurs publiques sont réduites à des réponses sûres 400/409 et ne divulguent ni détail interne, ni contenu, ni empreinte.
- Le parcours vertical crée, approuve puis exécute le plan en mock, vérifie la provenance, l'absence de contenu brut et de réseau Meta, puis prouve le masquage d'une extraction altérée. Les tests PostgreSQL à deux connexions vérifient le blocage de la suppression d'une pièce existante et de l'insertion d'une nouvelle pièce pendant la transaction.
- Preuves locales : ciblés 26 tests verts et 4 ignorés faute de PostgreSQL; suite exhaustive 152 fichiers/815 tests verts et 11 fichiers/28 tests ignorés; ESLint, TypeScript, build production avec valeurs factices du workflow, audit high, continuity-check et `git diff --check` verts. L'audit conserve trois avis modérés et aucun high/critical.
- Revue indépendante finale : aucun défaut P0/P1. Risques résiduels bornés : `schemaVersion: 1` devra être réévalué avant une API publique ou un déploiement mixte; un futur générateur réel devra interdire explicitement la recopie verbatim du contenu externe dans les champs métier. Le générateur déterministe courant ignore le contenu.
- La continuité `34430720259` est verte sur `bf54862`. La CI `34430720247` a validé audit, migrations, `db:verify`, sauvegarde/restauration, lint, TypeScript, 163 fichiers/844 tests PostgreSQL et le build. Dix-neuf Playwright sur vingt ont réussi; le seul échec était une attente de quatre projections alors que le parcours crée exactement deux routes sur le fil web/test et une sur le fil média WhatsApp.
- Le correctif E2E remplace l'attente par trois sans modifier le comportement applicatif. ESLint ciblé, TypeScript et diff check sont verts. Le commit `06137acf59e8d87f0deadd9d5da66aaa8b9c597c` est publié en fast-forward; CI `34432424191` et continuité `34432424276` sont en cours.
- État honnête : livré et publié; réel connecté = aucun; sandbox = aucune; mock = générateur/workflow et doubles média de test; bloqué humain = SMS Meta puis inventaire officiel et confirmation avant token; hors périmètre = Graph, stockage réel, antivirus, OCR/transcription, message externe, endpoint public, fusion, déploiement, dépense, CRM, Kanban, dashboard secondaire et OS-6.
- La continuité `34432424276` a réussi sur `06137ac`, mais la CI `34432424191` a échoué très tôt dans `db:verify`, avant lint, tests, build ou Playwright, avec PostgreSQL `57P01`. L'analyse a confirmé une course de fermeture : `pg-pool` pouvait résoudre `pool.end()` avant la disparition effective de la session TCP, puis le `DROP DATABASE ... WITH (FORCE)` supprimait sa propre connexion encore en fermeture.
- Le vérificateur de migrations attend maintenant de façon bornée que `pg_stat_activity` ne voie plus aucune session sur la base temporaire, puis exécute un `DROP DATABASE` sans `FORCE`. Si une session subsiste, il échoue explicitement au lieu d'avaler l'erreur. ESLint ciblé, TypeScript, le test RLS ciblé 11/11 et `git diff --check` sont verts; le correctif `f692057e284870527d7b162bae6cfa5312d9893c` a été publié en fast-forward sans inclure `tmp/`.
- La preuve finale est complète : continuité `34433025991` verte et CI `34433025988` entièrement verte sur `f692057`. Audit, migrations et nettoyage sans `57P01`, `db:verify`, sauvegarde/restauration, lint, TypeScript, 163 fichiers/844 tests PostgreSQL, build production et 20/20 Playwright ont réussi. La PR #11 reste ouverte, brouillon, fusionnable et `CLEAN`.
- État final de cette tranche : livré, publié et prouvé CI; réel connecté = aucun; sandbox = aucune; mock = générateur/workflow et doubles média uniquement; bloqué humain = saisie du SMS directement dans Meta puis inventaire officiel et confirmation avant token; hors périmètre = fournisseur réel, Graph, stockage réel, antivirus, OCR/transcription, message externe, endpoint public, fusion, déploiement, dépense, CRM, Kanban, dashboard secondaire et OS-6.

## 2026-09-11 - Garde anti-recopie et prévalidation des sorties de plan à 01:26 UTC

- Le travail applicatif a continué uniquement dans `/Users/TRADIKOM/Developer/TRADIKOM-ONE`; la copie iCloud n'a pas été utilisée. `tmp/` reste intact, non suivi et hors index. Le PDF maître est toujours exact : 71 pages et SHA-256 `bb838fb02c23247b1bcda8981539eebe73264a5334bfaf565aafa5bc26c50fe5`.
- Les pages cœur 3-7, 31-33, 46, 48 et 69-71 et les pages candidates OS-5 10-18, 22-24, 26-30, 35-38, 43-44 et 64-68 ont été relues directement; les pages 48 et 69 ont également été inspectées en rendu. `masterPrompt.alignment` a été actualisé avant le code.
- La garde pure et provider-agnostic inspecte les champs métier visibles, les preuves, la référence modèle, ainsi que les clés et valeurs récursives des entrées d'étape. Elle normalise Unicode, casse, accents, ponctuation et séparateurs, puis refuse la copie entière d'une source dès 32 caractères normalisés, une fenêtre substantielle de 48 caractères normalisés, un jeton dense ou des fragments cumulés distribués entre champs.
- Les valeurs structurelles fixes, comme les capacités et niveaux de risque, restent contrôlées individuellement mais sont exclues du cumul afin qu'une source qui les énumère ne bloque pas un plan légitime. Les marqueurs explicitement masqués et les courtes valeurs métier dérivées restent acceptés. Seules les métadonnées sûres de provenance sont exclues de l'inspection du plan persistant.
- Une revue indépendante a d'abord identifié qu'un getter à la racine d'une entrée pouvait être lu par Zod. Le correctif final `69b667f` projette profondément l'enveloppe du générateur, le plan puis les entrées depuis leurs descripteurs de données vers des copies JSON détachées avant Zod. Aucun getter ordinaire, trap `get` ou `toJSON` n'est invoqué.
- Les traps de réflexion d'un `Proxy` — `getPrototypeOf`, `ownKeys` et `getOwnPropertyDescriptor` — peuvent nécessairement s'exécuter pendant cette projection. Une réflexion défaillante est refusée sans propager son exception; la garantie n'est donc pas présentée comme une sandbox contre du code JavaScript hostile.
- Les graphes acycliques sérialisables en JSON avec références partagées restent acceptés. Cycles, accesseurs, prototypes personnalisés, symboles, clé `__proto__` normalisée, tableaux troués ou à index incohérents, nombres non finis et profondeurs excessives sont refusés. La projection est bornée à 200 000 propriétés et à 512 000 caractères cumulés de clés et chaînes pour le plan ou son enveloppe; chaque entrée d'étape est aussi bornée à 16 000 caractères de clés et chaînes ainsi qu'à 16 000 caractères une fois sérialisée.
- Un générateur adversarial qui recopie une source `external_untrusted_data` reçoit l'erreur publique française « Le plan n'a pas pu être préparé en toute sécurité. » Les tests causaux prouvent qu'aucun nouvel artefact lié au plan n'est créé : plan, étape, validation, message de type plan, ligne `workflow_runs` associée, événement `conversation.plan.execute` ou audit `conversation.plan_created`. Le message entrant source existe normalement déjà et le contenu comparé n'est jamais journalisé.
- Le parcours vertical conserve la canary dans l'extraction de la pièce jointe, visible par l'utilisateur autorisé. Il prouve son absence du panneau Plan et des sorties de plan inspectées : `plan_json`, `input_json`, messages plan/résultat et métadonnées d'audit. Cette vérification est une preuve de non-régression du chemin déterministe, distincte des tests causaux de rejet.
- Le correctif `0967ff1` crée un instantané JSON profond détaché du plan validé avant la garde et toute attente transactionnelle. Un test modifie la référence conservée au second `BEGIN` et prouve que ni la réponse ni `plan_json` ne changent; l'instantané n'est pas présenté comme profondément gelé.
- Les commits `a994b3a1836c0a7b2b74284a986975e58539dd1f`, `0967ff15e53a0bd0a3fb3c50e9fe03f97b89eb8f` puis `69b667fdc9502e44a6144d8046c343b5dd000fa0` ont été publiés strictement en fast-forward; local et distant sont synchronisés sur le head fonctionnel. `tmp/` n'a jamais été indexé.
- Preuves locales finales sur `69b667f` : 4 fichiers/49 tests ciblés verts; suite exhaustive 152 fichiers/842 tests verts et 11 fichiers/29 tests PostgreSQL ignorés faute de `DATABASE_URL`, soit 163 fichiers/871 tests au total. ESLint, TypeScript, build Next.js production avec valeurs factices, audit high, continuity-check et `git diff --check` verts. L'audit conserve trois avis modérés, zéro high/critical.
- La CI `34550617800` est entièrement verte sur `69b667f` en 23 min 55 s : audit avec trois avis modérés et zéro high/critical, migrations, `db:verify`, sauvegarde/restauration, lint, TypeScript, 163 fichiers/871 tests PostgreSQL inclus, build production Next.js 16.3.4 et 20/20 Playwright. La continuité `34550617974` est également verte; la PR #11 reste ouverte, brouillon, fusionnable et `CLEAN`.
- État honnête : livré, publié et prouvé CI; réel connecté = aucun fournisseur ou modèle et aucune clé réelle; sandbox = aucune; mock = générateur déterministe serveur et capacités de workflow mock au runtime, transports et doubles de modèle dans les tests, sans réseau fournisseur; bloqué humain = SMS Meta, inventaire officiel en lecture seule et confirmation avant token; hors périmètre = fournisseur IA réel, Graph, stockage média/objet réel, antivirus, OCR/transcription, message réel, endpoint public, fusion, déploiement, DNS, dépense, CRM, Kanban, dashboard secondaire et OS-6.
- Limites assumées : la garde détecte la recopie verbatim normalisée, pas une paraphrase sémantique, une obfuscation ou un encodage arbitraire, ni une PII ou un secret court sous les seuils. Avant un modèle réel, il faudra ajouter un squelette de capacités/policy produit côté serveur, une garde d'egress/DLP et des évaluations adversariales dédiées.
- Aucun fournisseur réel, secret, token, Graph, stockage média/objet réel, message externe, endpoint public, fusion, déploiement ou dépense n'a été déclenché. La prochaine action revient à la saisie humaine du SMS directement dans Meta, puis à l'inventaire officiel en lecture seule de l'application, du WABA et du Phone Number ID, et enfin à une confirmation distincte au moment exact avant tout token persistant stocké uniquement via référence serveur.

## 2026-09-11 - Validation unique stricte et confirmation d'état durable dans Conversation à 03:27 UTC

- Le développement applicatif a continué uniquement dans `/Users/TRADIKOM/Developer/TRADIKOM-ONE`; la copie iCloud n'a pas été utilisée et `tmp/` est resté intact, non suivi et hors index. Le PDF maître a été résolu dans Downloads, confirmé à 71 pages et au SHA-256 exact `bb838fb02c23247b1bcda8981539eebe73264a5334bfaf565aafa5bc26c50fe5`.
- Les pages cœur 3-7, 31-33, 46, 48 et 69-71 et les pages candidates 15-18, 35-38, 43-44 et 64-68 ont été relues directement; les pages 48 et 69 ont aussi été contrôlées en rendu. `masterPrompt.alignment` a été actualisé avant le code pour la première lacune concrète du jalon validation unique.
- La décision serveur n'interprète plus toute valeur autre que `approved` comme `rejected`. Une enum fermée est parsée à l'intérieur de `safeServerAction`; une valeur malformée produit une erreur française sûre avant le service et laisse zéro revalidation, redirection, audit ou message de décision.
- Les redirections de création, décision et exécution utilisent les identifiants réellement retournés par le service. La confirmation visuelle compare `plan_id` à un plan accessible du fil tenant, puis exige l'état durable correspondant : aucune mission pour créé/approuvé/refusé, mission `succeeded` pour exécuté. Un plan plus récent ne masque plus une confirmation cohérente du plan demandé.
- La confirmation demeure rejouable par URL et atteste uniquement l'état serveur courant; elle ne prouve ni la causalité de la navigation ni l'acteur de la transition et ne sert jamais d'autorisation.
- Le refus initial annule les deux étapes et rend l'exécution impossible. Le test de service prouve validation rejetée, zéro workflow, événement d'exécution, message résultat ou tâche, plus deux canaris non appelés sur le provider mock et `fetch`. L'audit de refus est comparé à une allowlist JSON exacte de quatre champs et n'inscrit jamais le motif métier.
- Le parcours Playwright desktop et mobile prépare puis refuse un deuxième plan via l'interface réelle, contrôle « Refusé », deux étapes « Annulée », l'absence des boutons Exécuter/Reprendre, le rejet d'un faux reçu exécuté, zéro requête Meta et zéro débordement horizontal. Les codes `crm.contacts.search` et `project.task.create` sont remplacés à l'écran par des libellés français.
- Les revues croisées ont fait corriger les reçus approuvé/refusé contradictoires en présence d'une mission, le masquage d'un reçu valide par un plan concurrent, la restauration du spy console et la preuve d'audit trop faible. La relecture finale ne conserve aucun défaut bloquant sur cette tranche.
- Preuves locales finales : 3 fichiers/27 tests ciblés verts; suite exhaustive 165 fichiers, 849 tests verts et 29 ignorés sans PostgreSQL local, soit 878 tests; canari provider ciblé vert après le dernier changement. ESLint, TypeScript, build Next.js production avec valeurs factices, audit high, continuity-check et `git diff --check` verts. L'audit garde trois avis modérés, aucun high/critical. Le Playwright local multiprocessus n'est pas présenté comme probant faute de PostgreSQL partagé.
- Le commit applicatif `dc9cf0c641bab56ac66d451aac5b18368ef8c12a` a été publié strictement en fast-forward depuis `897c5a0fa61ad03a00bc10fc3e6cc5aee0c9add7`. La continuité `34558477110` est verte. La CI `34558477111` a validé audit, migrations, sauvegarde/restauration, lint, TypeScript, les 165 fichiers/878 tests PostgreSQL et le build; son unique échec était un sélecteur Playwright qui cherchait le libellé sans son préfixe visible « 1. ».
- Les assertions ciblent maintenant les textes complets « 1. Rechercher le contact » et « 2. Préparer la tâche de suivi ». ESLint ciblé, TypeScript et diff check sont verts; le correctif `84a52a2bebf51c546e1f9f27dbf98dcb79dade20` a été publié en fast-forward. La continuité `34560151763` est verte sur ce head.
- La CI autoritative `34560151750` est entièrement verte sur `84a52a2` en 24 min 38 s : audit, migrations et `db:verify`, sauvegarde/restauration, lint, TypeScript, 165 fichiers/878 tests PostgreSQL, build production et 20/20 Playwright. La PR #11 reste ouverte, brouillon, fusionnable et `CLEAN`.
- État final de la tranche : livré, publié et prouvé CI; réel connecté = aucun; sandbox = aucune; mock = générateur déterministe et capacités `tradikom_mock`; bloqué humain = SMS Meta; hors périmètre = Graph, message réel, fusion, déploiement, dépense, CRM, Kanban, dashboard secondaire et OS-6. Aucun secret, token, Graph, message externe, endpoint public, fusion, déploiement ou dépense n'a été déclenché.
- Prochaine tranche non bloquée : produire un reçu de policy serveur append-only, immuable et versionné, fermer la préférence fournisseur par allowlist, puis revalider plan, fingerprint, validation, catalogue, fournisseur, scopes, rôle et risque avant exécution, appel direct du moteur, reprise et finalisation. Les plans historiques approuvés sans reçu resteront refusés fermé; le checkpoint SMS Meta reste parallèle et humain.

## 2026-09-11 - Reçu de policy immuable et enforcement durable OS-5 à 07:06 UTC

- Le travail applicatif a continué uniquement dans `/Users/TRADIKOM/Developer/TRADIKOM-ONE`; la copie iCloud n'a pas été utilisée et `tmp/` est resté intact, non suivi et hors index. Le PDF maître a été résolu dans Downloads, confirmé à 71 pages et au SHA-256 exact `bb838fb02c23247b1bcda8981539eebe73264a5334bfaf565aafa5bc26c50fe5`.
- Les pages cœur 3-7, 31-33, 46, 48 et 69-71, les pages métier 15-18, 35-38, 43-44, 64-68 et les pages techniques 10-12 et 22 ont été relues directement; les pages 48 et 69 ont aussi été inspectées en rendu. `masterPrompt.alignment` a été actualisé avant le code.
- Le catalogue de capacités est projeté vers une représentation canonique versionnée et profondément immuable. `providerPreference` n'accepte qu'un fournisseur unique, privé et canonique : `tradikom_mock`. Toute préférence inconnue, multiple ou porteuse d'un coût externe est refusée avant l'exécution.
- Un reçu de policy strictement serveur lie tenant, plan et fingerprint, validation éventuelle, catalogue et empreinte, capacités, fournisseur, scopes, rôle, principal et risque. Il est émis dans la même transaction que l'approbation, ou que la création pour `approval:none`; un rejet n'en reçoit jamais. Une ligne unique par plan est stockée sous RLS et les triggers rendent le reçu append-only et immuable.
- Les migrations 0110-0112 et leurs miroirs runtime ajoutent le reçu, ses politiques RLS et dix-huit politiques restrictives d'écriture sur les preuves Conversation. Un utilisateur ne peut pas forger plan, étape, validation, mission, événement ou reçu. La suppression indirecte d'un plan est refusée tant que son tenant existe; une suppression explicite du tenant reste possible et nettoie ses preuves.
- L'enforcement recalcule définition, étapes, validation, reçu, membership, rôle, scopes, catalogue, fournisseur, événement et mission avant l'exécution, un appel direct, chaque reprise, relance manuelle et finalisation. Une dérive échoue avant provider ou nouvel effet métier; un plan historique approuvé sans reçu reste fermé.
- Les contrôles génériques de workflow refusent les missions Conversation hors du service dédié. La finalisation vérifie la projection exacte des résultats et de l'audit. L'enqueue d'événement utilise un `INSERT ... ON CONFLICT DO NOTHING RETURNING`, ce qui accorde l'ownership à un seul appel concurrent.
- Le worker exécute les effets du handler et le passage de l'événement à `succeeded` dans une transaction atomique, avec savepoint s'il rejoint une transaction existante. Une revue indépendante a trouvé qu'après épuisement d'une reprise, l'annulation pouvait laisser la mission en `waiting`; le correctif final persiste atomiquement mission `failed`, étape terminale avec curseur sûr et lettre morte, puis prouve qu'une relance manuelle peut réussir.
- Preuves locales finales : matrice ciblée de 15 fichiers, 86 tests verts et 7 tests PostgreSQL ignorés faute de `DATABASE_URL`; suite exhaustive de 159 fichiers/874 tests verts et 13 fichiers/31 tests ignorés, soit 905 tests sans échec. ESLint, TypeScript, build Next.js production avec valeurs factices, audit high, continuity-check et `git diff --check` sont verts. L'audit conserve trois avis modérés (`qs`, `csv-parse`), zéro high/critical.
- Trois revues indépendantes ont couvert RLS, cascades, rôle runtime éphémère, atomicité, idempotence et récupération terminale; aucun défaut P1/P2 ne reste connu. Le commit applicatif local est `9879cae7e8bd0c9e598cd1c35ac55089f7a29b21`.
- État au checkpoint : livré et prouvé localement; publication fast-forward, CI PostgreSQL/RLS et Playwright encore requises. Réel connecté = aucun; sandbox = aucune; mock = générateur déterministe et capacités `tradikom_mock`; bloqué humain = SMS Meta, inventaire officiel puis confirmation immédiate avant token persistant; hors périmètre = Graph, message externe, endpoint public, fusion, déploiement, dépense, CRM, Kanban, dashboard secondaire et OS-6.

## 2026-09-11 - Preuve PostgreSQL/RLS et correction E2E ciblée à 07:34 UTC

- `9879cae` et le checkpoint `26cc3f1` ont été publiés strictement en fast-forward sans inclure `tmp/`. La continuité `34573066273` est verte.
- La CI `34573066265` a validé audit, migrations concurrentes/fresh/upgrade, `db:verify`, sauvegarde/restauration, lint, TypeScript, la suite exhaustive PostgreSQL avec preuves RLS et le build de production.
- Dix-neuf Playwright sur vingt ont réussi. L'unique échec attendait l'ancienne phrase « contact simulé retrouvé… » alors que l'application affichait correctement le nouveau résultat générique « toutes les étapes simulées ont été vérifiées », avec plan exécuté, étapes réussies et zéro effet externe.
- Le correctif `e704afd` modifie uniquement cette assertion visible. ESLint ciblé, TypeScript et `git diff --check` sont verts. Il doit être publié avec ce checkpoint puis confirmé par une nouvelle CI à 20/20 Playwright avant de classer la tranche entièrement prouvée CI.
- Aucun secret, token, Graph, fournisseur réel, message externe, endpoint public, fusion, déploiement ou dépense n'a été déclenché. Réel connecté = aucun; sandbox = aucune; mock = `tradikom_mock`; bloqué humain = SMS Meta; hors périmètre inchangé.

## 2026-09-14 - Révision immuable des plans Conversation à 22:39 UTC

- Le développement a repris sur l'application uniquement dans `/Users/TRADIKOM/Developer/TRADIKOM-ONE`; la copie iCloud n'a pas été utilisée et `tmp/` est resté intact, non suivi et hors index. Le PDF maître a été confirmé à 71 pages et au SHA-256 exact `bb838fb02c23247b1bcda8981539eebe73264a5334bfaf565aafa5bc26c50fe5`; les pages cœur 3-7, 31-33, 46, 48 et 69-71 ainsi que les pages candidates 10-12, 15-18, 22, 24, 35-38, 43-44 et 64-68 ont été relues directement, avec contrôle visuel des pages 48 et 69. `masterPrompt.alignment` a été actualisé avant le code.
- L'écran Conversation expose « Modifier » uniquement pour un plan en attente. Le formulaire français exige un titre réellement différent entre 3 et 160 caractères. Le service crée une nouvelle révision immuable liée au plan d'origine, annule atomiquement l'ancien plan, ses étapes et son approbation, régénère les clés d'idempotence, revalide catalogue et policy et crée une nouvelle approbation. Un plan déjà décidé ou remplacé est refusé fermé.
- L'ancien plan ne produit aucun nouveau reçu de policy, workflow, événement d'exécution, provider, résultat ou audit d'exécution. La nouvelle révision peut être approuvée puis exécutée indépendamment en `tradikom_mock`. Le rejeu résout la feuille courante, une concurrence à deux connexions ne crée qu'un successeur et la profondeur de lignée est bornée à 32.
- Le contrôle d'autorisation verrouille le plan puis le membership et le rôle dans la transaction avant toute mutation. Les migrations 0113 et runtime 119 ajoutent la lignée, les contraintes tenant-first, le fingerprint non nul et les triggers d'intégrité.
- Le durcissement complémentaire 0114/runtime 120 réserve le namespace orchestrateur à l'identité canonique exacte et ajoute 30 politiques RLS restrictives sur les preuves Conversation. Les fonctions sont `SECURITY INVOKER`; l'identité d'un message devient immuable. Les livraisons applicatives Twilio/Meta ordinaires restent autorisées : une migration historique et des changements de statut sont couverts sans appel réseau.
- Le parcours Playwright modifie un plan avant approbation, contrôle un nouvel identifiant, approuve et exécute la seule révision courante, puis prouve en base l'ancien plan refusé/annulé sans reçu, workflow, événement ou audit d'exécution. La variante mobile utilise un titre de 160 caractères et contrôle l'absence de débordement horizontal.
- Une revue croisée a identifié qu'après A→B puis B→C, rejouer la demande initiale A→B rendait B annulé. Un test dédié a reproduit l'échec avant correction; le service résout maintenant toute la lignée et rend C avec l'origine de rejeu A. La relecture finale confirme le correctif et ne conserve aucun P1/P2.
- Preuves locales finales : matrice ciblée 9 fichiers verts et 2 ignorés, 76 tests verts et 2 ignorés sans PostgreSQL; suite exhaustive mono-worker 160 fichiers verts et 14 ignorés, 896 tests verts et 32 ignorés sur 928. Un premier run parallèle a été abandonné après saturation locale et délais simultanés sur des modules indépendants; les sept fichiers signalés passent ensuite 7/7 et 10/10 en série, avant le passage exhaustif mono-worker en 831,47 secondes. ESLint, TypeScript, build Next.js production avec valeurs factices, audit high et diff check sont verts. Trois avis modérés restent suivis, zéro high/critical.
- État au checkpoint : livré et prouvé localement; publication fast-forward, CI PostgreSQL/RLS et 20/20 Playwright encore requises. Réel connecté = aucun; sandbox = aucune; mock = `tradikom_mock`; bloqué humain = SMS Meta puis autorisation distincte au moment exact; hors périmètre = Graph, message externe, endpoint public, fusion, déploiement, dépense, CRM, Kanban, dashboard secondaire et OS-6.
