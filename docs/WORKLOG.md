...
- Le commit fonctionnel `720db0e` est créé. Aucun compte, secret réel, Sandbox, endpoint public, message fournisseur, dépense, fusion ou déploiement n'a été créé. OS-5 reste `in_progress`; la prochaine tranche non bloquée est la consommation durable du budget d'autorisation avant tout checkpoint Sandbox réel.
