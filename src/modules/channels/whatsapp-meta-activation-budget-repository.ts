...
export type WhatsAppMetaActivationBudgetContextRow = {
...
export type WhatsAppMetaActivationConsumptionRow = {
  id: string;
  tenant_id: string;
  provider: "whatsapp_meta";
  endpoint_id: string;
...
export async function lockMetaWhatsAppActivationBudgetContext(
  ...
  const result = await db.query<WhatsAppMetaActivationBudgetContextRow>(
    `select authz.id as authorization_id,
     ...
     join channel_provider_deliveries delivery
      ...
      and delivery.activation_authorization_id = authz.id
...
export async function lockMetaWhatsAppDeliveryForActivationBudget(
  db: DbClient,
  input: { tenantId: string; endpointId: string; deliveryId: string },
) {
  const result = await db.query<{
    id: string;
    activation_authorization_id: string | null;
  }>(
    `select id, activation_authorization_id
     ...
     for update`,
    [input.tenantId, input.endpointId, input.deliveryId],
  );
  return result.rows[0] ?? null;
}

export async function findMetaWhatsAppConsumptionByDelivery(
  db: DbClient,
  input: { tenantId: string; deliveryId: string },
) {
  const result = await db.query<WhatsAppMetaActivationConsumptionRow>(
...
export async function countMetaWhatsAppActivationConsumptions(
  db: DbClient,
...
export async function insertMetaWhatsAppActivationConsumption(
  db: DbClient,
  input: WhatsAppMetaActivationConsumptionRow,
) {
  const result = await db.query<WhatsAppMetaActivationConsumptionRow>(
    `insert into channel_provider_activation_consumptions (
     ...
     ) values ($1, $2, 'whatsapp_meta', $3, $4, $5, $6, $7)
     returning *`,
    [
      input.id,
      input.tenant_id,
