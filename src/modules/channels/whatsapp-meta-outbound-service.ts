...
const provider = "whatsapp_meta" as const;
const adapterKey = "whatsapp-meta";
...
const metaOutboundRequestSchema = z
  .object({
...
const metaOutboundAttemptSchema = z
  .object({
    tenantId: boundedIdentifierSchema,
...
export type WhatsAppMetaOutboundPolicyContext = {
  ...
  providerState: WhatsAppMetaOutboundAdapter["manifest"]["state"];
};

export type WhatsAppMetaOutboundPolicyEvaluator = (
  context: WhatsAppMetaOutboundPolicyContext,
) =>
  | { allowed: true }
  | { allowed: false; code: string }
  | Promise<{ allowed: true } | { allowed: false; code: string }>;

export type WhatsAppMetaOutboundDependencies = {
  adapter: WhatsAppMetaOutboundAdapter;
...
export class WhatsAppMetaOutboundError extends Error {
  constructor(
    public readonly code:
      | "whatsapp_meta_outbound_context_not_found"
      | "whatsapp_meta_outbound_context_invalid"
      | "whatsapp_meta_outbound_idempotency_conflict"
      | "whatsapp_meta_outbound_delivery_not_found",
    message: string,
  ) {
    super(message);
    this.name = "WhatsAppMetaOutboundError";
  }
}

/**
 * Réserve puis tente un envoi Meta. Hors mock, l'adaptateur refuse avant tout
 * transport; la preuve durable reste alors disponible dans la conversation.
 */
export async function sendPreparedMetaWhatsAppOutbound(
  db: DbClient,
  actorId: string,
  input: z.input<typeof metaOutboundRequestSchema>,
    ...
    async (transaction) => {
      ...
      if (!reservation.replayed) {
        await recordAuditLog(transaction, {
          tenantId: parsed.tenantId,
          actorId,
          action: "channel.whatsapp_meta_outbound_reserved",
  ...
  return attemptPreparedMetaWhatsAppOutboundDelivery(
    db,
    actorId,
...
export async function attemptPreparedMetaWhatsAppOutboundDelivery(
  db: DbClient,
  actorId: string,
      ...
      if (!isValidContext(context, claimed.status === "failed")) {
        const invalid = await finalizeClaimedWhatsAppOutboundDelivery(
          ...
          terminalOutcome(
            ...
            attemptedAtIso,
            "validation",
            context?.endpoint_status === "disabled"
              ? "channel_endpoint_inactive"
              : undefined,
          ),
        );
        if (!invalid) throw deliveryNotFound();
        await requireMessageStateUpdate(transaction, invalid, "failed");
...
async function evaluatePolicySafely(
  evaluator: WhatsAppMetaOutboundPolicyEvaluator,
  context: WhatsAppMetaOutboundPolicyContext,
) {
  try {
...
function assertValidContext(
  ...
  if (!context) {
    throw new WhatsAppMetaOutboundError(
      "whatsapp_meta_outbound_context_not_found",
      "Le contexte de livraison WhatsApp Meta est introuvable.",
    );
  }
  if (!isValidContext(context, allowFailedMessage)) {
    throw new WhatsAppMetaOutboundError(
      "whatsapp_meta_outbound_context_invalid",
      "Le contexte de livraison WhatsApp Meta n'est pas autorisé.",
    );
  }
}
...
function isValidContext(
  ...
  return Boolean(
    context &&
      ...
      context.identity_adapter_key === adapterKey &&
...
async function recordAttemptAudit(
  ...
  actorId: string,
  delivery: ChannelProviderDeliveryRow,
  transportState: WhatsAppMetaOutboundAdapter["manifest"]["state"],
) {
  await recordAuditLog(db, {
    tenantId: delivery.tenant_id,
    actorId,
    action: "channel.whatsapp_meta_outbound_attempted",
    targetType: "channel_provider_delivery",
...
async function recordCompletionAudit(
  ...
  const action =
    delivery.status === "accepted" || delivery.status === "delivered"
      ? "channel.whatsapp_meta_outbound_succeeded"
      : delivery.retryable
        ? "channel.whatsapp_meta_outbound_retry_scheduled"
        : delivery.status === "denied"
          ? "channel.whatsapp_meta_outbound_denied"
          : "channel.whatsapp_meta_outbound_failed";
  await recordAuditLog(db, {
    tenantId: delivery.tenant_id,
    actorId,
    action,
...
function assertMatchingFingerprint(
  ...
  if (delivery.request_fingerprint !== requestFingerprint) {
    throw new WhatsAppMetaOutboundError(
      "whatsapp_meta_outbound_idempotency_conflict",
      "La clé d'idempotence correspond à un autre envoi WhatsApp Meta.",
    );
  }
}
...
async function requireMessageStateUpdate(
  ...
  if (!updated) {
    throw new WhatsAppMetaOutboundError(
      "whatsapp_meta_outbound_context_invalid",
      "Le message WhatsApp Meta ne peut pas être réconcilié.",
    );
  }
}
...
function deliveryNotFound() {
  return new WhatsAppMetaOutboundError(
    "whatsapp_meta_outbound_delivery_not_found",
    "La livraison WhatsApp Meta est introuvable.",
  );
}
